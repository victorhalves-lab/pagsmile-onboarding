                  ...
                  <TableRow key={p.id} className="hover:bg-[#f4f4f4] transition-colors">
                    <TableCell>
                      <div className="flex items-center gap-1.5">
                        <span className="font-mono text-sm text-[#2bc196]">{p.codigo || '-'}</span>
                        {(p.version || 1) > 1 && <span className="text-[9px] bg-[#2bc196]/10 text-[#2bc196] px-1.5 py-0.5 rounded font-bold">v{p.version}</span>}
                      </div>
                    </TableCell>
                    <TableCell className="font-medium text-sm">{p.clienteNome || '-'}</TableCell>
                    <TableCell className="text-sm text-[#002443]/60">{p.clienteCnpj || '-'}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <span className={`text-xs ${expiring ? 'text-amber-600 font-medium' : 'text-[#002443]/60'}`}>{p.validUntil ? moment(p.validUntil).format('DD/MM/YY') : '-'}</span>
                      </div>
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {isFinalized && <Button variant="ghost" size="sm" onClick={() => criarNovaVersao(p)} title="Nova versão"><GitBranch className="w-4 h-4 text-[#2bc196]" /></Button>}
