...
export default function ProcessosModelo() {
  ...
  return (
    <div className="space-y-6">
      {/* Hero Header */}
      <div className="bg-gradient-to-r from-[#002443] to-[#003a66] rounded-2xl p-6 text-white">
        ...
        <div className="mt-4">
          <Button
            ...
            className="bg-[#2bc196] hover:bg-[#2bc196]/90 text-white text-xs gap-2 px-4 py-2"
          >
            ...
            {filtered.map(p => (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 rounded-xl border transition-all text-xs ${
                  selectedId === p.id
                    ? 'bg-[#2bc196]/10 border-[#2bc196]/30 text-[#002443]'
                    : 'bg-white border-slate-200 text-[#002443]/70 hover:border-[#2bc196]/20 hover:bg-[#2bc196]/5'
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <Badge className="bg-[#002443]/10 text-[#002443] border-0 text-[8px] font-mono shrink-0">{p.id}</Badge>
                      <span className="text-[9px] text-[#002443]/40">v{p.versao}</span>
                    </div>
                    <p className="font-semibold truncate text-[11px]">{p.nome}</p>
                    <p className="text-[9px] text-[#002443]/40 mt-0.5 truncate">{p.area}</p>
                  </div>
                  <ChevronRight className={`w-3.5 h-3.5 shrink-0 ${selectedId === p.id ? 'text-[#2bc196]' : 'text-slate-300'}`} />
                </div>
              </button>
            ))}
          </div>
          ...
          {selected ? (
            ...
            <div className="flex items-center justify-center h-96 bg-white rounded-2xl border border-slate-200">
              <div className="text-center">
                ...
                <p className="text-sm font-semibold text-[#002443]/40">Selecione um processo à esquerda</p>
                <p className="text-xs text-[#002443]/20 mt-1">{PROCESSOS.length} processos documentados no formato padrão v2.0</p>
              </div>
            </div>
          )}
        </div>
