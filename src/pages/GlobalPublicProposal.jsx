...
export default function GlobalPublicProposal() {
  ...
  if (loading) {
    return (
      <PublicGlobalShell title="Pagsmile Global" lang={lang} setLang={setLang}>
        <div className="p-12 text-center text-[#002443]/60">{t('loading')}</div>
      </PublicGlobalShell>
    );
  }
    ...
    return (
      <PublicGlobalShell title="Pagsmile Global" lang={lang} setLang={setLang}>
        <div className="p-10 text-center">
          ...
          <p className="text-[#002443]/70">{t('prop_not_found')}</p>
        </div>
      </PublicGlobalShell>
    );
  }
    ...
    return (
      <PublicGlobalShell title={t('prop_title')} lang={lang} setLang={setLang}>
        <div className="p-10 text-center">
          ...
          <h2 className="text-xl font-bold text-[#002443] mb-2">{t('prop_rejected_title')}</h2>
          <p className="text-[#002443]/70">{t('prop_rejected_desc')}</p>
        </div>
      </PublicGlobalShell>
    );
  }
    ...
    return (
      <PublicGlobalShell title={t('prop_title')} lang={lang} setLang={setLang}>
        <div className="p-10 text-center">
          ...
          <h2 className="text-xl font-bold text-[#002443] mb-2">{t('prop_counter_sent')}</h2>
          <p className="text-[#002443]/70">{t('prop_counter_sent_desc')}</p>
        </div>
      </PublicGlobalShell>
    );
  }
  ...
  return (
    <PublicGlobalShell title={t('prop_title')} lang={lang} setLang={setLang} maxWidth="4xl">
      <div className="p-6 md:p-8 space-y-6">
        {/* Intro */}
        <p className="text-[#002443]/70">{t('prop_intro', { name: proposal.contact_name || proposal.client_name })}</p>
        ...
        {/* Breakdown */}
        <div className="bg-[#f4f4f4]/50 rounded-2xl p-5 border border-[#002443]/5">
          <h3 className="text-xs font-bold uppercase tracking-wider text-[#002443]/60 mb-3">{t('prop_breakdown')}</h3>
        ...
        {/* Detalhes adicionais */}
        <div className="text-xs text-[#002443]/60 space-y-1">
          <div>{t('prop_refund')}: <strong className="text-[#002443]">{fmtUsd(proposal.refund_fee)}</strong></div>
          {proposal.risk_control_fee != null && <div>{t('prop_risk')}: <strong className="text-[#002443]">{fmtUsd(proposal.risk_control_fee)}</strong></div>}
          {proposal.valid_until && <div>{t('prop_valid_until')}: <strong className="text-[#002443]">{proposal.valid_until}</strong></div>}
          {proposal.mccs?.length > 0 && <div>MCCs: <strong className="text-[#002443]">{proposal.mccs.join(', ')}</strong></div>}
          {proposal.target_markets?.length > 0 && <div>Markets: <strong className="text-[#002443]">{proposal.target_markets.join(', ')}</strong></div>}
        </div>

        {/* Ações */}
        <div className="flex flex-wrap gap-3 pt-4 border-t border-[#002443]/5">
          <Button onClick={accept} className="bg-[#2bc196] hover:bg-[#2bc196]/90">
...
function HeroBox({ icon: Icon, label, value, accent }) {
  return (
    <div className={`rounded-2xl p-5 border ${accent ? 'border-[#2bc196]/30 bg-gradient-to-br from-[#2bc196]/5 to-[#5cf7cf]/5' : 'border-[#002443]/5 bg-white'}`}>
      <div className="flex items-center gap-2 mb-2">
        <Icon className="w-4 h-4 text-[#2bc196]" />
        <span className="text-[10px] font-bold uppercase tracking-wider text-[#002443]/60">{label}</span>
      </div>
      <div className="text-3xl font-bold font-mono text-[#002443]">{value}</div>
    </div>
  );
}
function InfoBox({ icon: Icon, label, value }) {
  return (
    <div className="bg-white rounded-xl border border-[#002443]/5 p-3">
      <div className="flex items-center gap-1.5 mb-1">
        <Icon className="w-3 h-3 text-[#002443]/40" />
        <span className="text-[9px] font-bold uppercase tracking-wider text-[#002443]/50">{label}</span>
      </div>
      <div className="text-sm font-semibold text-[#002443]">{value}</div>
    </div>
  );
}
function BreakdownItem({ label, value }) {
  return (
    <div>
      ...
      <div className="font-mono text-sm text-[#002443] mt-0.5">{value}</div>
    </div>
  );
}
