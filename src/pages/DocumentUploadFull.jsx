...
export default function DocumentUploadFull() {
  // Prioridade: URL (?model=XXX) > localStorage > default
  // Isto permite que o link funcione mesmo após "limpar cache" ou reload direto.
  const urlParams = new URLSearchParams(window.location.search);
  const urlModel = urlParams.get('model');
  const urlCaseId = urlParams.get('caseId');
  const urlTemplateId = urlParams.get('templateId');

  // Detecta se há QUALQUER contexto utilizável (URL ou localStorage).
  const hasContext = !!(
    urlModel ||
    urlTemplateId ||
    urlCaseId ||
  ...
  const resolvedModel = urlModel || resolveCurrentModel() || 'ComplianceEcommerceV4';
  const config = getComplianceModelConfig(resolvedModel);

  // Persistir no localStorage para que as próximas etapas (CAF, upload) continuem
  // funcionando mesmo se a URL for perdida em navegação subsequente.
  ...
  // Sem contexto nenhum: mostra tela de recuperação ao invés de mandar o cliente
  // cego para um template default errado.
  if (!hasContext) {
