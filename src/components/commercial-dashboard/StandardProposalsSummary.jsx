  ...
  return (
    <Card className="rounded-2xl border-[#002443]/5">
      ...
      <CardContent className="space-y-4">
        {/* Standard by segment */}
        <div>
          <p className="text-xs font-semibold text-[#002443]/50 uppercase mb-2">Propostas Padrão Ativas por Segmento</p>
          {Object.keys(segmentCounts).length === 0 ? (
            <p className="text-xs text-[#002443]/30">Nenhuma proposta padrão ativa</p>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(segmentCounts).sort((a, b) => b[1] - a[1]).map(([seg, count]) => (
                <div key={seg} className="flex items-center justify-between p-2 rounded-lg bg-[#f4f4f4]">
                  <span className="text-xs font-medium text-[#002443]">{seg}</span>
                  <span className="text-xs font-bold text-[#002443]">{count}</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* PIX summary */}
        <div>
          <p className="text-xs font-semibold text-[#002443]/50 uppercase mb-2">Propostas PIX</p>
          <div className="grid grid-cols-3 gap-2">
            <div className="text-center p-2 rounded-lg bg-[#f4f4f4]">
              <p className="text-lg font-bold text-[#002443]">{pixTotal}</p>
              <p className="text-[10px] text-[#002443]/40">Total</p>
            </div>
            <div className="text-center p-2 rounded-lg bg-blue-50">
              <p className="text-lg font-bold text-blue-600">{pixSent}</p>
              <p className="text-[10px] text-[#002443]/40">Enviadas</p>
            </div>
