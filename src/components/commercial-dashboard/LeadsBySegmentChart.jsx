...
export default function LeadsBySegmentChart({ data }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5">
      <div className="flex items-center gap-2 mb-4">
        <Layers className="w-4 h-4 text-[#002443]/70" />
        <h3 className="font-bold text-[#002443]">Leads por Segmento</h3>
      </div>
      <ResponsiveContainer width="100%" height={280}>
        <BarChart layout="vertical" data={data} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
          ...
          <Bar dataKey="count" fill="#002443" radius={[0, 4, 4, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
