    ...
    <div className="space-y-5">
      ...
      <div className="border-l-4 border-purple-500 bg-purple-50/30 rounded-r-2xl p-5 space-y-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-[#0A0A0A]">Fase 1 — Análise do Questionário</h4>
            <p className="text-xs text-[#0A0A0A]/50">Score: score_questionario (0-1000) · Classificação: classificacao_questionario</p>
          </div>
        </div>
      ...
      <div className="border-l-4 border-blue-500 bg-blue-50/30 rounded-r-2xl p-5 space-y-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-[#0A0A0A]">Fase 2 — Validações Externas</h4>
            <p className="text-xs text-[#0A0A0A]/50">Score: score_validacao_externa (0-1000) · Classificação: classificacao_validacao_externa</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
      ...
      {/* FASE 3 */}
      <div className="border-l-4 border-[#1356E2] bg-[#1356E2]/5 rounded-r-2xl p-5 space-y-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#1356E2] flex items-center justify-center text-white font-bold">F3</div>
          <div>
            <h4 className="font-bold text-[#0A0A0A]">Fase 3 — Score Composto + Recomendação Final</h4>
            <p className="text-xs text-[#0A0A0A]/50">Score: score_geral_composto (0-1000) · Recomendação: recomendacao_final</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <p className="text-[10px] font-bold text-[#1356E2] uppercase tracking-wider mb-2">Cálculo do Score Composto</p>
            {['Combina score_questionario + score_validacao_externa com pesos configuráveis', 'Aplica bonus_consistencia (0-1000): premia quando dados declarados = dados externos', 'Aplica overrides automáticos: PEP detectado → desconto automático, CNPJ irregular → desconto, biometria falha → penalidade', 'Resultado: score_geral_composto = (SQ × peso1 + SVE × peso2 + Bônus) / fator_normalização', 'Classificação final: classificacao_geral baseada em thresholds do template'].map((item, i) => (
              <p key={i} className="text-[10px] text-[#0A0A0A]/60 flex items-start gap-1.5 mb-1">
                <CircleDot className="w-3 h-3 text-[#1356E2] mt-0.5 shrink-0" />{item}
              </p>
            ))}
          </div>
          <div>
            <p className="text-[10px] font-bold text-[#1356E2] uppercase tracking-wider mb-2">Recomendação Final e Artefatos</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <p className="text-[10px] font-bold text-[#0A0A0A]/50 uppercase tracking-wider mb-3">Decisão Automática — Thresholds por Template</p>
