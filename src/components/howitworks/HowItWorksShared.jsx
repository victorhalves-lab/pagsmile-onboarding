    ...
    <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white mb-4 shadow-sm hover:shadow-md transition-shadow">
      <button onClick={() => toggleSection(id)} className="w-full flex items-center justify-between p-5 hover:bg-slate-50/50 transition-colors">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-[#2bc196]/10 rounded-xl"><Icon className="w-5 h-5 text-[#2bc196]" /></div>
...
export function FlowStep({ number, title, description, icon: Icon, details, color = "bg-[#2bc196]" }) {
  return (
          ...
          <div className="mt-2 pl-4 border-l-2 border-[#2bc196]/20 space-y-1">
            {details.map((d, i) => (
              <p key={i} className="text-xs text-[#002443]/60 flex items-start gap-1.5">
                <CircleDot className="w-3 h-3 text-[#2bc196] mt-0.5 shrink-0" />{d}
              </p>
            ))}
          </div>
        )}
...
export function FeatureCard({ title, description, icon: Icon, items, color = "bg-[#002443]/5" }) {
  return (
          ...
          <ul className="space-y-1.5">
            {items.map((item, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-[#002443]/80">
                <CheckCircle2 className="w-4 h-4 text-[#2bc196] mt-0.5 flex-shrink-0" /><span>{item}</span>
              </li>
            ))}
          </ul>
        )}
  ...
  return (
    <div className="border border-slate-200 rounded-xl p-5 hover:border-[#2bc196]/30 transition-colors">
      <div className="flex items-start justify-between mb-3">
        ...
        <Badge className={access === 'Público' ? 'bg-blue-100 text-blue-700 border-0' : 'bg-[#002443] text-white border-0'}>{access}</Badge>
      </div>
      <div className="bg-slate-50 rounded-xl p-4 mb-3">
        <h5 className="text-xs font-bold text-[#002443]/50 uppercase tracking-wider mb-2.5">Funcionalidades</h5>
        <ul className="space-y-2">
          {funcionalidades.map((f, j) => (
            <li key={j} className="flex items-start gap-2 text-sm text-[#002443]/80">
              <ArrowRight className="w-3.5 h-3.5 text-[#2bc196] mt-0.5 flex-shrink-0" /><span>{f}</span>
            </li>
          ))}
        </ul>
      </div>
      {subAbas && subAbas.length > 0 && (
        <div className="bg-[#002443]/5 rounded-xl p-4">
          <h5 className="text-xs font-bold text-[#002443]/50 uppercase tracking-wider mb-2.5">Sub-abas / Seções Internas</h5>
          <ul className="space-y-1.5">
            {subAbas.map((t, i) => (
              <li key={i} className="text-xs text-[#002443]/60 flex items-start gap-1.5">
                <Hash className="w-3 h-3 text-[#002443]/30 mt-0.5 shrink-0" />{t}
              </li>
            ))}
          </ul>
        </div>
