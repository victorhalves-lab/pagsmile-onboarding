...
const PageDoc = ({ name, route, description, purpose, access, funcionalidades, subAbas, fluxo, entidadesAfetadas }) => (
  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6">
    ...
    <div className="p-4 space-y-4">
      ...
      {subAbas?.length > 0 && (
        <div>
          <h5 className="text-xs font-bold text-[#0A0A0A] uppercase tracking-wider mb-1.5">📑 Seções Internas ({subAbas.length})</h5>
      ...
      {fluxo?.length > 0 && (
        <div>
          <h5 className="text-xs font-bold text-[#0A0A0A] uppercase tracking-wider mb-2">🔀 Fluxograma</h5>
      ...
      {entidadesAfetadas?.length > 0 && (
        <div>
          <h5 className="text-xs font-bold text-[#0A0A0A] uppercase tracking-wider mb-1.5">🗄️ Entidades</h5>
    ...
    <div className="space-y-6">
      ...
      <PageDoc
        ...
        funcionalidades={[
          ...
          'Seção 5 — Anexos e Cláusulas (ContratoAnexos): reserva risco PIX (% e dias), reserva risco cartão (% e dias + liberação parcial), chargeback (taxa R$ e threshold %), duração contrato (meses), multa rescisão (% e valor máx), dados Pin Bank (representante, cargo, CPF), testemunhas (2 nomes + CPFs), cláusulas customizadas',
