...
export default function ConteudoContrato({ contract }) {
  ...
  return (
    <div style={{ 
      padding: '48px 56px', color: '#0A0A0A', fontSize: '10.5px', lineHeight: 1.75, 
      ...
      <div data-pdf-block="header" style={{ 
        background: 'linear-gradient(135deg, #0A0A0A 0%, #003366 50%, #E84B1C 100%)',
        ...
        <p style={{ color: '#E84B1C', fontSize: '11px', fontWeight: 500, letterSpacing: '1px', position: 'relative' }}>
          ABERTURA DE CONTA DE PAGAMENTO E SUBADQUIRÊNCIA
        </p>
        <div style={{ width: '60px', height: '3px', background: '#1356E2', margin: '16px auto 0', borderRadius: '2px' }} />
      </div>

      {/* ===== PREÂMBULO ===== */}
      <P>Pelo presente instrumento particular, de um lado,</P>
      <P><strong>PIN BANK INSTITUICAO DE PAGAMENTO LTDA</strong>, pessoa jurídica de direito privado, inscrita no CNPJ sob o nº 37.753.531/0001-65, com sede na Alameda Santos, 1940 - Cerqueira Cesar, São Paulo - SP, 01.418-102, neste ato devidamente representada na forma de seus atos constitutivos, doravante denominada simplesmente <strong>"PIN BANK"</strong>;</P>
      <P>E, de outro lado,</P>
      ...
      <P>PIN BANK e CONTRATANTE, quando em conjunto, serão doravante denominadas "PARTES", e, individualmente, "PARTE".</P>
      ...
      {c.customClauses && (
        <>
          ...
          <div style={{ marginBottom: '24px', textAlign: 'justify', whiteSpace: 'pre-wrap', fontSize: '10.5px', lineHeight: 1.75, color: '#0A0A0A' }}>{c.customClauses}</div>
        </>
      )}

      {/* ===== ASSINATURAS ===== */}
      <div data-pdf-block="signatures" style={{ borderTop: '3px solid #1356E2', paddingTop: '32px', marginTop: '48px' }}>
        ...
        <p style={{ textAlign: 'center', marginBottom: '32px', fontSize: '11px', color: '#0A0A0A' }}>
          São Paulo, <V value={formatDate(c.contractDate)} />.
        </p>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', marginBottom: '32px' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ borderBottom: '2px solid #0A0A0A', marginBottom: '12px', paddingBottom: '48px' }} />
            <p style={{ fontWeight: 700, fontSize: '11px', color: '#0A0A0A', marginBottom: '4px' }}>PIN BANK INSTITUICAO DE PAGAMENTO LTDA</p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>CONTRATADA</p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>Nome: <V value={c.pagsmileRepresentativeName || 'Ricardo Winter'} /></p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>Cargo: <V value={c.pagsmileRepresentativeRole || 'CFO'} /></p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>CPF: <V value={c.pagsmileRepresentativeCPF} /></p>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ borderBottom: '2px solid #0A0A0A', marginBottom: '12px', paddingBottom: '48px' }} />
            <p style={{ fontWeight: 700, fontSize: '11px', color: '#0A0A0A', marginBottom: '4px' }}><V value={c.clientName} placeholder="[NOME COMPLETO DO CONTRATANTE]" /></p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>CONTRATANTE</p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>Nome: <V value={c.clientRepresentativeName} /></p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>Cargo: <V value={c.clientRepresentativeRole} /></p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>CPF: <V value={c.clientRepresentativeCPF} /></p>
          </div>
        </div>

        <p style={{ fontWeight: 700, marginBottom: '16px', fontSize: '11px', color: '#0A0A0A' }}>TESTEMUNHAS:</p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ borderBottom: '2px solid #0A0A0A', marginBottom: '12px', paddingBottom: '32px' }} />
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>1. Nome: <V value={c.witness1Name} /></p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>CPF: <V value={c.witness1CPF} /></p>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ borderBottom: '2px solid #0A0A0A', marginBottom: '12px', paddingBottom: '32px' }} />
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>2. Nome: <V value={c.witness2Name} /></p>
            <p style={{ fontSize: '10px', color: '#0A0A0A', margin: '2px 0' }}>CPF: <V value={c.witness2CPF} /></p>
          </div>
        </div>

        {/* Footer */}
        <div style={{ marginTop: '40px', paddingTop: '16px', borderTop: '1px solid rgba(0,36,67,0.08)', textAlign: 'center' }}>
          <img src={LOGO_URL} alt="Pin Bank" style={{ height: '28px', opacity: 0.3 }} />
          <p style={{ fontSize: '9px', color: '#0A0A0A', opacity: 0.3, marginTop: '8px' }}>
            &copy; {new Date().getFullYear()} Pin Bank Instituição de Pagamento Ltda. Todos os direitos reservados.
          </p>
        </div>
      </div>
    </div>
