...
export default function ContratoAnexos({ contract }) {
  ...
  return (
    <>
      ...
      <P>1.1. Os Serviços de Gateway e Orquestrador de Pagamento consistem na disponibilização, pela PIN BANK, de uma infraestrutura tecnológica robusta e segura, destinada ao processamento, roteamento inteligente e gestão de Transações com cartões de crédito e débito, bem como outros meios de pagamento eletrônico. O serviço inclui a conectividade com múltiplos adquirentes e a aplicação de regras de negócio para otimização de taxas de aprovação e custos.</P>
      ...
      <P>3.2. Notificado pela PIN BANK sobre a ocorrência de um Chargeback, o CONTRATANTE deverá realizar a análise da contestação e apresentar toda a documentação necessária para a disputa (reapresentação) em um prazo máximo de 7 (sete) dias corridos, contados da data da notificação. A falha em apresentar a documentação neste prazo poderá resultar na perda do direito de disputa e na assunção do prejuízo pelo CONTRATANTE.</P>
      ...
      <P>1.1. Constitui condição essencial e indispensável para a abertura e a manutenção da Conta de Pagamento junto à PIN BANK que o CONTRATANTE seja uma pessoa jurídica regularmente constituída, com inscrição válida e em situação ativa no Cadastro Nacional da Pessoa Jurídica (CNPJ) no momento do cadastro e durante toda a vigência contratual. A irregularidade cadastral do CNPJ poderá ensejar o bloqueio ou encerramento da conta.</P>

      <ClauseTitle>ANEXO I-C – MÓDULO COBRANÇA PIX (API PIX PARA COBRANÇA)</ClauseTitle>
      <SubClauseTitle>1. OBJETO ESPECÍFICO</SubClauseTitle>
      <P>1.1. O presente Módulo tem por objeto a disponibilização, pela PIN BANK ao CONTRATANTE, de uma Interface de Programação de Aplicações (API) específica para a geração e gestão de cobranças via PIX. O serviço permite a criação de QR Codes estáticos e dinâmicos, a definição de datas de vencimento e outros parâmetros de cobrança, e a conciliação automática dos pagamentos recebidos, através da notificação em tempo real (webhook) e de relatórios específicos.</P>
      ...
      {c.ratesSourceName && (
        <P style={{ fontSize: '9.5px', color: '#0A0A0A', opacity: 0.5, fontStyle: 'italic', marginBottom: '8px' }}>
      ...
      <P>3.1. Em contrapartida aos preços e condições comerciais pactuados neste Instrumento, o CONTRATANTE compromete-se a manter, durante a vigência contratual, um volume mínimo mensal de transações processadas através da plataforma da PIN BANK, conforme os patamares descritos na tabela abaixo.</P>
      <BrandTable
      ...
      <P>Constituem atividades expressamente proibidas na plataforma PIN BANK, cuja prática pelo CONTRATANTE configurará infração contratual grave, sujeita à rescisão imediata por justa causa e demais sanções cabíveis. Esta lista é exemplificativa e não exaustiva.</P>
      ...
      <P><em>PARÁGRAFO ÚNICO: A PIN BANK poderá, a seu critério, classificar outras atividades como proibidas com base em sua análise de risco, em alterações na legislação aplicável ou em determinações de autoridades competentes, notificando o CONTRATANTE a respeito.</em></P>

      {/* ===== ANEXO IV ===== */}
      <SectionHeading>ANEXO IV – SLA + SUPORTE</SectionHeading>
      <SubClauseTitle>1. NÍVEIS DE SERVIÇO (SLA)</SubClauseTitle>
      <P>1.1. Disponibilidade da Plataforma: A PIN BANK se compromete a manter a plataforma de produção disponível para processamento de transações em, no mínimo, <V value={c.slaUptime || '99.5%'} /> do tempo, apurado mensalmente.</P>
      <P>1.2. Tempo de Resposta da API: O tempo médio de resposta da API da PIN BANK para o processamento síncrono de Transações de autorização não deverá exceder <V value={c.slaResponseTime || '600ms'} />, medido no servidor da PIN BANK.</P>
      ...
      <P>1.2. PIN BANK como Operadora: No contexto da prestação dos serviços objeto deste Contrato e em relação aos dados dos Consumidores Finais do CONTRATANTE, a PIN BANK é qualificada como Operadora. A PIN BANK realizará o tratamento dos dados pessoais em nome do Controlador (CONTRATANTE), em estrita conformidade com as suas instruções lícitas e documentadas, e para as finalidades exclusivas de processamento de pagamentos, prevenção a fraudes e cumprimento de obrigações regulatórias, conforme permitido pela legislação.</P>

      <SubClauseTitle>2. OBRIGAÇÕES DAS PARTES</SubClauseTitle>
      <P>2.1. A PIN BANK, como Operadora, compromete-se a tratar os dados pessoais estritamente para as finalidades autorizadas pelo CONTRATANTE e necessárias à execução do Contrato, a não compartilhar tais dados com terceiros sem instrução expressa, e a garantir que seus colaboradores e suboperadores envolvidos no tratamento estejam sujeitos a obrigações de confidencialidade.</P>
      <P>2.2. Compete ao CONTRATANTE, como Controlador, garantir que possui uma base legal válida para todo o tratamento de dados pessoais realizado, bem como gerenciar e responder às solicitações dos titulares de dados relativas ao exercício de seus direitos, contando com a cooperação da PIN BANK para tanto, naquilo que lhe for aplicável.</P>
      ...
      <KVTable items={[
        ...
        ['API', 'Interface de programação de aplicações que permite a integração segura e automatizada entre os sistemas do CONTRATANTE e a plataforma da PIN BANK.'],
        ...
        ['Consumidor Final', 'Pessoa física ou jurídica que adquire produtos ou serviços do CONTRATANTE e realiza o pagamento através da plataforma PIN BANK.'],
        ['Conta de Pagamento', 'Conta de titularidade do CONTRATANTE, aberta e mantida junto à PIN BANK, destinada à movimentação de recursos e à realização de pagamentos.'],
        ['Estabelecimento', 'Cliente do CONTRATANTE, pessoa física ou jurídica, para o qual o CONTRATANTE utiliza os serviços da PIN BANK para processar pagamentos.'],
        ...
        ['MDR', 'Taxa de Desconto do Lojista, expressa como um percentual cobrado pela PIN BANK sobre o valor de cada transação com cartão.'],
        ...
        ['Transação', 'Qualquer operação de pagamento, transferência ou movimentação de fundos processada através da Plataforma PIN BANK.'],
        ['Usuário Master', 'Pessoa física devidamente indicada e autorizada pelo CONTRATANTE para acessar o Aplicativo da PIN BANK e gerenciar a Conta de Pagamento.'],
      ]} />
    </>
  );
}