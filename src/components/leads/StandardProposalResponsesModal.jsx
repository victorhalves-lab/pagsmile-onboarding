...
function DistributionBar({ dist }) {
  ...
  const items = [
    ...
    { label: 'PIX', value: dist.pix, color: '#1356E2' },
      ...
      <div className="flex gap-4">
        {items.map(i => (
          <div key={i.label} className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#0A0A0A]/70">{i.label}: <strong>{i.value}%</strong></span>
          </div>
        ))}
      </div>
    </div>
                ...
                {activeSection === 'volumetria' ? (
                  <div className="space-y-5">
                    ...
                    <div className="rounded-xl bg-[#f8fafc] border border-[#e2e8f0] p-5">
                      <p className="text-xs text-[#0A0A0A]/55 font-medium mb-3">Distribuição do TPV</p>
