...
function ResponseValue({ question }) {
  ...
  if (formatted === null) return <span className="text-[#0A0A0A]/30 italic text-sm">Não informado</span>;
    ...
    return (
      <div className="flex flex-wrap gap-1.5">
        {formatted.map((item, i) => (
          <span key={i} className="inline-flex px-3 py-1.5 rounded-lg bg-[#1356E2]/8 border border-[#1356E2]/15 text-xs font-semibold text-[#0A0A0A]">
            {item}
          </span>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {Object.entries(value).map(([label, pct]) => (
          <div key={label} className="flex items-center gap-3">
            <span className="text-xs font-medium text-[#0A0A0A]/70 min-w-[160px]">{shortName(label)}</span>
            <div className="flex-1 max-w-[200px] h-5 bg-slate-100 rounded-full overflow-hidden">
              <div className="h-full bg-[#1356E2] rounded-full transition-all" style={{ width: `${Math.min(parseFloat(pct) || 0, 100)}%` }} />
            </div>
            <span className="text-sm font-bold text-[#0A0A0A] w-12 text-right">{parseFloat(pct) || 0}%</span>
          </div>
        ))}
        <div className="text-xs text-[#0A0A0A]/40 pt-1 border-t border-slate-100">Total: {total.toFixed(0)}%</div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {Object.entries(value).map(([key, val]) => (
          <div key={key} className="bg-slate-50 rounded-lg px-3 py-2 border border-slate-100">
            <span className="text-[10px] text-[#0A0A0A]/50 block">{rateLabels[key] || key}</span>
  ...
  return (
    <p className={`text-sm text-[#0A0A0A] font-medium leading-relaxed ${isLongText ? 'whitespace-pre-wrap' : ''}`}>
      {formatted}
    </p>
  );
}
                  ...
                  <div>
                    ...
                    {currentSectionConfig && (
                      <div className="flex items-center gap-3 mb-5 pb-4 border-b border-[#e2e8f0]">
                        ...
                        <div>
                          <h3 className="text-base font-bold text-[#0A0A0A]">{currentSectionConfig.label}</h3>
                    ...
                    {activeSec === 'arquivos' ? (
                      filteredQuestions.length === 0 ? (
                        <div className="text-center py-16">
                          <Paperclip className="w-12 h-12 mx-auto text-[#0A0A0A]/10 mb-3" />
                          <p className="text-[#0A0A0A]/30 text-sm">Nenhum arquivo enviado</p>
                        </div>
                      ) : (
                        <div className="space-y-3">
                          {filteredQuestions.map(q => (
                            <div key={q.id} className="flex items-center justify-between bg-[#f8fafc] rounded-xl px-5 py-4 border border-[#e2e8f0] hover:border-[#1356E2]/30 transition-colors">
                              <div className="flex items-center gap-3 min-w-0">
                                <div className="w-10 h-10 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center justify-center shrink-0">
                                  <Paperclip className="w-4 h-4 text-emerald-600" />
                                </div>
                                <div className="min-w-0">
                                  <p className="text-sm text-[#0A0A0A] font-semibold truncate">{q.text}</p>
                                  <p className="text-[11px] text-[#0A0A0A]/40 truncate font-mono">{q.value ? String(q.value).split('/').pop() : 'Não enviado'}</p>
                                </div>
                              </div>
                              {q.value && (
                                <div className="flex gap-1.5 shrink-0 ml-3">
                                  <a href={String(q.value)} target="_blank" rel="noopener noreferrer">
                                    <Button size="sm" className="bg-[#1356E2]/10 text-[#1356E2] hover:bg-[#1356E2]/20 border-0 h-8 text-xs rounded-lg font-semibold">
                                      <ExternalLink className="w-3 h-3 mr-1" /> Abrir
                                    </Button>
                                  </a>
                                  <a href={String(q.value)} download>
                                    <Button size="sm" variant="ghost" className="text-[#0A0A0A]/40 hover:text-[#0A0A0A] h-8 text-xs rounded-lg">
                      ...
                      filteredQuestions.length === 0 && !showCardRates && !showDistribution ? (
                        <div className="text-center py-16">
                          <FileText className="w-12 h-12 mx-auto text-[#0A0A0A]/10 mb-3" />
                          <p className="text-[#0A0A0A]/30 text-sm">Nenhuma pergunta nesta seção</p>
                        </div>
                      ) : (
                        <div className="space-y-3">
                          {filteredQuestions.map((q, idx) => (
                            <div key={q.id}
                              className={`group relative rounded-xl transition-all duration-150 ${
                                q.hasValue
                                  ? 'bg-[#f8fafc] hover:bg-[#f1f5f9] border border-[#e2e8f0] hover:border-[#1356E2]/30'
                                ...
                                <div className="flex items-start justify-between gap-4">
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mb-2">
                                      <span className="text-[10px] font-bold text-[#0A0A0A]/25 tabular-nums">
                                        #{String(idx + 1).padStart(2, '0')}
                                      </span>
                                      <p className="text-xs text-[#0A0A0A]/60 font-medium leading-relaxed">{q.text}</p>
                                      {!q.hasValue && (
                                  ...
                                  {q.hasValue && (
                                    <button onClick={() => copyValue(q.id, q.value)}
                                      ...
                                      {copiedId === q.id
                                        ...
                                        : <Copy className="w-4 h-4 text-[#0A0A0A]/25" />}
                                    </button>
                                  )}
                                </div>
                              </div>
