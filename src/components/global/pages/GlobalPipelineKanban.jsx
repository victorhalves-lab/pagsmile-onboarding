...
const COLUMNS = [
  ...
  { id: 'proposal_accepted', label: 'Aceita',            color: 'border-t-[#2bc196]'   },
...
export default function GlobalPipelineKanban() {
  ...
  if (isLoading) {
    return <div className="bg-white rounded-2xl border border-[#002443]/5 shadow-sm p-10 text-center text-[#002443]/50">Carregando pipeline...</div>;
  }
            ...
            <Droppable key={col.id} droppableId={col.id}>
              {(provided, snapshot) => (
                <div
                  ...
                  className={`bg-[#f4f4f4]/50 rounded-2xl border-t-4 ${col.color} ${snapshot.isDraggingOver ? 'bg-[#2bc196]/10' : ''} min-w-[200px]`}
                >
                  <div className="px-3 py-2 border-b border-[#002443]/5">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-[#002443]">{col.label}</h3>
                      <span className="text-[10px] font-bold text-[#002443]/50 bg-white px-2 py-0.5 rounded-full">{colItems.length}</span>
                    </div>
                    <div className="text-[10px] text-[#002443]/50 mt-0.5">TPV: {fmt(tpv)}</div>
                  </div>
                  ...
                  <div className="p-2 space-y-2 min-h-[200px]">
                      ...
                      <Draggable key={q.id} draggableId={q.id} index={idx}>
                        {(prov, snap) => (
                          <div
                            ...
                            className={`bg-white rounded-xl border border-[#002443]/5 p-3 shadow-sm cursor-grab ${snap.isDragging ? 'shadow-lg rotate-1' : ''}`}
                          >
                            <div className="flex items-start gap-2">
                              <Building2 className="w-3.5 h-3.5 text-[#2bc196] mt-0.5 flex-shrink-0" />
                              <div className="min-w-0 flex-1">
                                <div className="text-xs font-semibold text-[#002443] truncate">{q.company_name}</div>
                                <div className="text-[10px] text-[#002443]/50 truncate flex items-center gap-1 mt-0.5">
                                  <Mail className="w-2.5 h-2.5" /> {q.contact_email}
                                </div>
                                <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#002443]/5">
                                  <span className="text-[10px] font-mono text-[#2bc196] flex items-center gap-1">
                                    <DollarSign className="w-2.5 h-2.5" />{fmt(q.monthly_tpv)}
                                  </span>
                                  <span className="text-[9px] text-[#002443]/40">
                    ...
                    {colItems.length === 0 && (
                      <div className="text-center text-[10px] text-[#002443]/30 py-6">Vazio</div>
                    )}
                  </div>
                </div>
              )}
