...
/**
 * Template PDF estilo "Pagsmile Pricing Proposal - Roblox": SEM capa.
...
export default function CountryPricingPdfTemplate({ proposal, lang = 'en' }) {
  const i18n = {
    en: {
      ...
      intl_settlement_text: 'After deducting the fees from the payments processed for merchant\'s website, PAGSMILE will transfer the money internationally to merchant\'s bank account. The sums received will be withheld the related fees.',
      intl_settlement_freq: 'PAGSMILE will settle ONCE per each {days} calendar days in {currency} the sums received, with the related fees deducted.',
      valid_until: 'Valid until', footer: 'Pagsmile Limited · www.pagsmile.com · Unit 8B, Wing Hang Insurance Building, 11 Wing Kut Street, Central, Hong Kong',
    },
    pt: {
      ...
      intl_settlement_text: 'Após deduzir as taxas dos pagamentos processados pelo site do merchant, a PAGSMILE transferirá o dinheiro internacionalmente para a conta bancária do merchant. Os valores recebidos terão as taxas relacionadas deduzidas.',
      intl_settlement_freq: 'A PAGSMILE liquidará UMA vez a cada {days} dias corridos em {currency} os valores recebidos, com as taxas relacionadas deduzidas.',
      valid_until: 'Válida até', footer: 'Pagsmile Limited · www.pagsmile.com · Unit 8B, Wing Hang Insurance Building, 11 Wing Kut Street, Central, Hong Kong',
    },
    zh: {
      ...
      intl_settlement_text: '从商户网站处理的付款中扣除费用后,PAGSMILE 将把资金国际转账到商户的银行账户。收到的金额将扣除相关费用。',
      intl_settlement_freq: 'PAGSMILE 将每 {days} 个日历日以 {currency} 结算一次收到的金额(扣除相关费用)。',
      valid_until: '有效期至', footer: 'Pagsmile Limited · www.pagsmile.com · Unit 8B, Wing Hang Insurance Building, 11 Wing Kut Street, Central, Hong Kong',
    },
  };
  ...
  return (
    <div style={{
      ...
      color: '#002443', padding: 48,
    }}>
      {/* Cabeçalho compacto (sem capa) */}
      <div style={{ marginBottom: 28, paddingBottom: 16, borderBottom: '2px solid #2bc196' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 22, fontWeight: 800 }}>Pagsmile</div>
            <div style={{ fontSize: 11, color: '#002443', opacity: 0.6, marginTop: 2 }}>Pricing Proposal</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#002443', opacity: 0.5, textTransform: 'uppercase', letterSpacing: 1 }}>{t.proposal_for}</div>
            <div style={{ fontSize: 16, fontWeight: 700 }}>{proposal.client_name || '—'}</div>
            <div style={{ fontSize: 11, color: '#002443', opacity: 0.6 }}>{proposal.contact_name} · {proposal.contact_email}</div>
          </div>
        </div>
      </div>
        ...
        return (
          <div key={country.country} style={{ marginBottom: 36, breakInside: 'avoid' }}>
            ...
            {split.payin.length > 0 && <>
              <div style={{ fontSize: 11, color: '#002443', opacity: 0.7, marginBottom: 8 }}>{t.payin_fee}</div>
            ...
            {(country.taxes || []).map((tax, i) => {
              ...
              return (
                <div key={i} style={{ fontSize: 11, fontStyle: 'italic', color: '#002443', opacity: 0.7, marginTop: 8 }}>
                  {labelByLang || tax.label || `${tax.tax_type}: ${tax.percentage}%`}
                </div>
              );
            })}
            {(country.restrictions || []).map((r, i) => (
              <div key={i} style={{ fontSize: 11, fontStyle: 'italic', color: '#002443', opacity: 0.7, marginTop: 4 }}>
                *{r}
              </div>
            ))}
            ...
            {split.payout.length > 0 && <>
              <div style={{ fontSize: 11, color: '#002443', opacity: 0.7, marginTop: 16, marginBottom: 8 }}>{t.payout_fee}</div>
      ...
      {/* Other Fees */}
      <div style={{ marginTop: 32, paddingTop: 20, borderTop: '2px solid #2bc196', breakInside: 'avoid' }}>
      ...
      {/* Footer */}
      <div style={{ marginTop: 40, paddingTop: 16, borderTop: '1px solid rgba(0,36,67,0.08)', display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#002443', opacity: 0.55 }}>
...
function Table({ headers, rows }) {
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
      <thead>
        <tr style={{ background: '#2bc196', color: '#ffffff' }}>
          {headers.map((h, i) => (
...
function Fee({ label, value }) {
  return (
    <div style={{ padding: 10, border: '1px solid rgba(0,36,67,0.1)', borderRadius: 8 }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: '#002443', opacity: 0.55, textTransform: 'uppercase', letterSpacing: 0.5 }}>{label}</div>
