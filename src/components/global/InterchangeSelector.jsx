...
export default function InterchangeSelector({ value, onChange, customPct, customFixed, onCustomChange }) {
  const groups = [
    ...
    { brand: 'combined', label: 'Combined',     data: INTERCHANGE_SUMMARY.combined, color: 'from-[#1356E2]/8 to-[#E84B1C]/8', border: 'border-[#1356E2]/30' },
  ];
        ...
        {groups.map(g => (
          <div key={g.brand} className={`rounded-xl border ${g.border} bg-gradient-to-br ${g.color} p-3`}>
            <div className="flex items-center gap-1.5 mb-2">
              <CreditCard className="w-3.5 h-3.5 text-[#0A0A0A]/60" />
              <span className="text-[11px] font-bold text-[#0A0A0A]">{g.label}</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-2 rounded-lg border transition-all ${
                      isActive
                        ? 'bg-[#1356E2] text-white border-[#1356E2] shadow-sm'
                        : 'bg-white text-[#0A0A0A] border-[#0A0A0A]/10 hover:border-[#1356E2]/40'
                    }`}
                  >
      ...
      <button
        ...
        className={`w-full p-3 rounded-xl border-2 border-dashed transition-all flex items-center gap-3 ${
          value === 'custom'
            ...
            : 'border-[#0A0A0A]/15 hover:border-[#1356E2]/40 bg-white'
        }`}
      >
        <Sparkles className={`w-4 h-4 ${value === 'custom' ? 'text-[#1356E2]' : 'text-[#0A0A0A]/40'}`} />
        <div className="text-left flex-1">
          <div className="text-xs font-bold text-[#0A0A0A]">Custom</div>
          <div className="text-[10px] text-[#0A0A0A]/60">Define seus próprios valores de interchange</div>
        </div>
        {value === 'custom' && (
          <div className="flex gap-2" onClick={e => e.stopPropagation()}>
            <input
              ...
              className="w-20 h-8 rounded-md border border-[#0A0A0A]/15 px-2 text-xs font-mono"
              placeholder="%"
            />
            <input
