...
function VariableRow({ variable, direction }) {
  ...
  return (
    <div className={`flex items-start gap-2.5 p-2 rounded-lg ${isNeg ? 'bg-red-50/40' : 'bg-emerald-50/40'}`}>
      ...
      {desc.category && (
        <span className="text-[9px] uppercase tracking-wider font-bold text-[#0A0A0A]/40 shrink-0">
          {desc.category}
        </span>
      )}
    </div>
...
export default function ScoreBreakdownExplainer({ complianceScore, segmento }) {
  ...
  return (
    <div className="rounded-xl border border-[#0A0A0A]/10 bg-slate-50/50 overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-2.5 p-3 hover:bg-slate-50 transition-colors text-left"
      >
        <BookOpen className="w-4 h-4 text-[#0A0A0A]/60" />
        <div className="flex-1 min-w-0">
          <p className="text-[12px] font-bold text-[#0A0A0A]">Entenda o Score V4</p>
      ...
      {open && (
        <div className="px-3 pb-3 space-y-3 border-t border-[#0A0A0A]/8 pt-3">
          {/* C1 */}
          <LayerCard
            layer="C1"
          ...
          >
            {hasAnyVar ? (
              ...
              <p className="text-[11px] text-[#0A0A0A]/50 italic py-1">Variáveis aplicadas não disponíveis para este caso.</p>
            )}
          </LayerCard>
          ...
          >
            {positiveVars.length > 0 ? (
              ...
              <p className="text-[11px] text-[#0A0A0A]/50 italic py-1">Nenhum enriquecimento positivo identificado neste caso.</p>
            )}
          </LayerCard>

          {/* Footer: total + rule */}
          <div className="p-3 rounded-lg bg-[#0A0A0A] text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Layers className="w-4 h-4 text-[#1356E2]" />
              ...
              <div className="flex items-baseline gap-1 font-mono">
                ...
                <span className="text-[#1356E2] font-black text-lg">{total}</span>
              </div>
            </div>
...
function LayerCard({ layer, title, value, sign, explanation, items, children }) {
  ...
  const iconColor = sign === 'negative' ? 'text-red-500' : sign === 'positive' ? 'text-emerald-500' : 'text-[#0A0A0A]/40';
  const valueColor = sign === 'negative' ? 'text-red-700' : sign === 'positive' ? 'text-emerald-700' : 'text-[#0A0A0A]';

  return (
    <div className={`rounded-lg border ${tone} p-3 space-y-2`}>
      <div className="flex items-center gap-2">
        <span className="font-mono text-[10px] font-bold bg-[#0A0A0A] text-white px-2 py-0.5 rounded">{layer}</span>
        <span className="text-[12px] font-bold text-[#0A0A0A] flex-1">{title}</span>
        <Icon className={`w-3.5 h-3.5 ${iconColor}`} />
        <span className={`font-mono text-sm font-black ${valueColor}`}>
          {value > 0 ? '+' : ''}{value} pts
        </span>
      </div>
      <p className="text-[11px] text-[#0A0A0A]/60 leading-relaxed">{explanation}</p>
      {items && items.length > 0 && (
        <div className="pt-1 border-t border-current/10 space-y-1">
          {items.map((it, i) => (
            <div key={i} className="flex justify-between text-[11px]">
              <span className="text-[#0A0A0A]/70">{it.label}</span>
              <span className="font-mono font-bold text-[#0A0A0A]">{it.value}</span>
            </div>
          ))}
        </div>
      )}
