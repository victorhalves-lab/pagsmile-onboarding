...
function RateField({ label, value, onChange, suffix = '%', prefix = '' }) {
  return (
    <div>
      <label className="text-[11px] font-semibold text-[#002443]/60 mb-1 block">{label}</label>
      <div className="relative">
        {prefix && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-[#002443]/40 font-medium">{prefix}</span>}
        <Input
          type="number"
          step="0.01"
          min="0"
        ...
        />
        {suffix && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[#002443]/40 font-medium">{suffix}</span>}
      </div>
    </div>
  );
}

function SectionHeader({ icon: Icon, title, color = '#002443' }) {
  return (
...
export default function SegmentRateForm({ data, onChange }) {
  ...
  return (
    <div className="space-y-6">
      {/* Informações Gerais */}
      <div className="bg-white rounded-xl border border-[#002443]/8 p-5">
        <SectionHeader icon={FileText} title="Informações do Segmento" color="#002443" />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="text-[11px] font-semibold text-[#002443]/60 mb-1 block">Nome do Segmento</label>
            <Input value={data.segmentName || ''} onChange={e => update('segmentName', e.target.value)} className="text-sm font-semibold h-9" />
          </div>
          <div>
            <label className="text-[11px] font-semibold text-[#002443]/60 mb-1 block">MCC</label>
            <Input value={data.mcc || ''} onChange={e => update('mcc', e.target.value)} className="text-sm font-semibold h-9" />
          </div>
          <div>
            <label className="text-[11px] font-semibold text-[#002443]/60 mb-1 block">Nível de Risco</label>
      ...
      <div className="bg-white rounded-xl border border-[#002443]/8 p-5">
        <SectionHeader icon={CreditCard} title="Taxas de Cartão de Crédito (MDR)" color="#2bc196" />
      ...
      {/* Antecipação */}
      <div className="bg-white rounded-xl border border-[#002443]/8 p-5">
        <SectionHeader icon={Repeat} title="Antecipação de Recebíveis" color="#2bc196" />
      ...
      {/* PIX */}
      <div className="bg-white rounded-xl border border-[#002443]/8 p-5">
        <SectionHeader icon={Smartphone} title="PIX" color="#2bc196" />
      ...
      {/* Taxas Adicionais */}
      <div className="bg-white rounded-xl border border-[#002443]/8 p-5">
        <SectionHeader icon={ReceiptText} title="Taxas Adicionais" color="#002443" />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
