      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b-2 border-[#2bc196]/20">
            <th className="text-left py-2 px-2 font-semibold text-[#002443]/70">Parcela</th>
            <th className="text-right py-2 px-2 font-semibold text-[#002443]/70">Base</th>
            {showAntecipacao && <th className="text-right py-2 px-2 font-semibold text-amber-600">Antecipação</th>}
            <th className="text-right py-2 px-2 font-semibold text-[#2bc196]">Taxa Final</th>
          </tr>
        </thead>
        <tbody>
          {rows.map(r => (
            <tr key={r.parcela} className="border-b border-[#002443]/5 hover:bg-[#2bc196]/5">
              ...
              <td className="py-1.5 px-2 text-right font-bold text-[#2bc196]">{r.taxaFinal.toFixed(2)}%</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-sm font-bold text-[#002443]">Pré-visualização das Taxas</h3>
      </div>

      {/* MDR Summary */}
      <div className="bg-[#002443] rounded-xl p-5 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-40 h-40 bg-[#2bc196]/8 rounded-full blur-[60px]" />
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <div key={r.label} className="bg-white/[0.06] border border-white/[0.08] rounded-lg px-2 py-3 text-center">
                ...
                <p className="text-lg font-extrabold" style={{ color: '#2bc196' }}>
                  {fmtPct(r.value)}
                </p>
              </div>
            ))}
          ...
          {/* Antecipação */}
          <div className="mt-3 bg-[#2bc196]/10 border border-[#2bc196]/20 rounded-lg p-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Repeat className="w-3.5 h-3.5" style={{ color: '#2bc196' }} />
              <span className="text-xs font-bold" style={{ color: '#ffffff' }}>Antecipação</span>
            </div>
            <span className="text-lg font-extrabold" style={{ color: '#2bc196' }}>{fmtPct(data.percentualAntecipacao)} <span className="text-[10px]" style={{ color: 'rgba(43,193,150,0.6)' }}>a.m.</span></span>
          </div>
        </div>
      </div>

      {/* PIX */}
      <div className="bg-white border border-[#2bc196]/20 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-3">
          <Smartphone className="w-4 h-4 text-[#2bc196]" />
          <span className="text-xs font-bold text-[#002443]">PIX</span>
        </div>
        <div className="flex items-center justify-center gap-6">
          <div className="text-center">
            <p className="text-[10px] font-semibold text-[#002443]/50 mb-0.5">Percentual</p>
            <p className="text-xl font-extrabold text-[#2bc196]">{fmtPct(data.pixTaxaPercentual)}</p>
          </div>
          <span className="text-xs font-bold text-[#002443]/20">ou</span>
          <div className="text-center">
            <p className="text-[10px] font-semibold text-[#002443]/50 mb-0.5">Fixa</p>
            <p className="text-xl font-extrabold text-[#2bc196]">{fmtBrl(data.pixTaxaFixa)}</p>
          </div>
        </div>
      </div>

      {/* Taxas Adicionais */}
      <div className="bg-white border border-[#002443]/8 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-3">
          <ReceiptText className="w-4 h-4 text-[#002443]/50" />
          <span className="text-xs font-bold text-[#002443]">Taxas Adicionais</span>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={item.label} className="bg-[#f4f4f4] rounded-lg p-2.5 text-center">
              <p className="text-[10px] font-semibold text-[#002443]/50">{item.label}</p>
              <p className="text-sm font-bold text-[#002443]">{item.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border border-[#002443]/8 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold text-[#002443]">Simulação de Parcelas</span>
        </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${
                prazo === opt.value
                  ...
                  : 'bg-[#f4f4f4] text-[#002443]/50 hover:bg-[#2bc196]/10 hover:text-[#002443]'
              }`}
            >
              {opt.label}
            </button>
