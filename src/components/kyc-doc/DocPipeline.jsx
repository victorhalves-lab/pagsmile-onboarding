...
export default function DocPipeline() {
  return (
    <S>
      ...
      <InfoBox title="Pipeline paralelo do Lead — roda ANTES do pipeline KYC" color="blue">
        <p>Quando o cliente preenche um funil de captação (Lead V5, PIX V4 ou Fechamento — Seção 19), um pipeline próprio de 4 etapas roda via <code>onLeadCreatedEnrich</code>: (1) <code>bdcEnrichLead</code> — datasets BDC agressivos com basic_data, kyc, owners_kyc, processes, activity_indicators, domains, addresses, financial_market; (2) <code>bdcDeepDueLead</code> — deep due diligence quando TPV &gt; R$500k OU segmento ∈ &#123;gateway, marketplace, plataforma_vertical&#125;; (3) <code>analyzeLeadQualifier</code> — IA classifica maturidade comercial; (4) <code>analyzeLeadRiskAdvanced</code> — análise preliminar de risco com IA. <strong>Os dados BDC coletados aqui são REUTILIZADOS</strong> pela função <code>bdcEnrichCase</code> (Step 1 abaixo) quando o Lead converte em <code>OnboardingCase</code> — cache hit por CNPJ evita chamada duplicada à BDC. A análise preliminar de IA do Lead é apenas comercial/consultiva, NÃO tem poder regulatório — quem decide o KYC é exclusivamente o pipeline KYC abaixo (Data-First v7).</p>
      </InfoBox>
