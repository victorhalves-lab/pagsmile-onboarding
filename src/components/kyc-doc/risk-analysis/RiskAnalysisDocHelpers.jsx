...
export function DocH1({ num, children }) {
  return (
    <h1 className="text-[18pt] font-black text-[#002443] mt-8 mb-3 pb-2 border-b-2 border-[#2bc196] leading-tight print-h1">
      {num && <span className="text-[#2bc196] font-mono text-[11pt] mr-2">{num}</span>}
      {children}
    </h1>
  );
}
...
export function DocH2({ num, children }) {
  return (
    <h2 className="text-[14pt] font-black text-[#002443] mt-6 mb-2 leading-tight">
      {num && <span className="text-[#2bc196] font-mono text-[10pt] mr-2">{num}</span>}
      {children}
    </h2>
  );
}
...
export function DocH3({ children }) {
  return <h3 className="text-[12pt] font-bold text-[#2bc196] mt-4 mb-1.5">{children}</h3>;
}
...
export function DocCode({ children }) {
  return (
    <code className="font-mono text-[9.5pt] bg-[#f0f4f8] text-[#002443] px-1 py-0.5 rounded">
      {children}
    </code>
  );
}
        ...
        <thead>
          <tr className="border-b-2 border-[#2bc196]">
            {headers.map((h, i) => (
              <th
                key={i}
                className="text-left text-[9pt] font-bold text-[#002443] uppercase tracking-wider py-2 px-2 align-bottom"
              >
                {h}
              </th>
            ))}
...
export function BlockCard({ n, title, source, children }) {
  return (
    <div className="my-4 border border-[#e8e8e8] rounded-lg overflow-hidden print-avoid-break">
      <div className="bg-[#002443] text-white px-4 py-2.5 flex items-center gap-3">
        <div className="w-7 h-7 rounded-full bg-[#2bc196] text-white font-black text-[12pt] flex items-center justify-center shrink-0">
          {n}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[11pt] font-bold leading-tight">{title}</p>
          {source && <p className="text-[8.5pt] text-[#5cf7cf] font-mono mt-0.5">↳ {source}</p>}
        </div>
      </div>
