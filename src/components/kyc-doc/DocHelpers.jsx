...
export const S = ({ children }) => <div className="kyc-doc-section bg-white rounded-2xl border border-[#002443]/8 px-8 py-6 print:px-4">{children}</div>;
...
export const Table = ({ headers, rows }) => (
  <div className="overflow-x-auto my-4">
    <table className="w-full text-xs border-collapse border border-[#002443]/10 print:text-[9px]">
      <thead><tr className="kyc-doc-table-header bg-[#002443] text-white">{headers.map((h,i) => <th key={i} className="px-3 py-2 text-left font-semibold border border-[#002443]/20 text-white">{h}</th>)}</tr></thead>
...
export const InfoBox = ({ title, children, color = 'blue' }) => {
  ...
  return <div className={`kyc-doc-infobox p-4 rounded-xl border ${c[color]||c.blue} my-4 print:p-2`}><p className="text-xs font-bold text-[#002443] mb-1.5">{title}</p><div className="text-xs text-[#002443]/70 leading-relaxed space-y-1">{children}</div></div>;
};
