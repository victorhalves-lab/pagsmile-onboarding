...
export default function DocVisaoGeral() {
  return (
    <S>
      ...
      <H2>1.3. As 7 Etapas do Processo Completo (Lead → KYC → Decisão)</H2>
      <P>O processo end-to-end começa <strong>antes</strong> do KYC formal — pelo funil de captação que produz a entidade <code>Lead</code> com enriquecimento BDC preliminar e classificação comercial pela IA. As 7 etapas, nesta exata ordem:</P>
      <ol className="list-decimal ml-6 space-y-3 mb-4">
        <Li><Bold>Etapa 0 — Captação do Lead (Pré-KYC):</Bold> O cliente preenche um dos 3 funis públicos ativos: <em>Lead Pin Bank V5</em> (12 etapas, cartão), <em>Lead PIX V4</em> (7 etapas, PIX) ou <em>Fechamento Landing</em> (3 blocos consolidados, vindo de proposta padrão). O submit dispara automaticamente <code>onLeadCreatedEnrich</code> que: (a) consulta BDC com datasets agressivos (<code>bdcEnrichLead</code>), (b) faz deep due diligence se o lead é alto valor (<code>bdcDeepDueLead</code>), (c) classifica maturidade comercial via IA (<code>analyzeLeadQualifier</code>), (d) faz análise preliminar de risco com IA (<code>analyzeLeadRiskAdvanced</code>). O Lead resultante já carrega TPV declarado, score qualificador, BDC enrichment e classificação de risco preliminar — dados que serão reutilizados (cache) pela Etapa 4 abaixo. <em>Detalhes microscópicos: Seção 19.</em></Li>
        <Li><Bold>Etapa 1 — Questionário de Compliance:</Bold> Após o lead aceitar uma proposta, o cliente é convertido para <code>OnboardingCase</code> e preenche o questionário V4 específico ao seu segmento (<code>ComplianceEcommerceV4</code>, <code>ComplianceGatewayV4</code>, <code>CompliancePixMerchantV4</code>, etc.). Respostas do Lead V5/PIX V4 são <em>pré-populadas</em> automaticamente em <code>QuestionnaireResponse</code> — cliente não responde duas vezes. O questionário coleta dados societários e operacionais, com autocomplete CNPJ via Receita Federal, autocomplete CEP via ViaCEP, lógica condicional e autosave.</Li>
