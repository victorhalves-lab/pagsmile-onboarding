...
export default function SecurityAuditTable({ twoFactor, loginAttempts }) {
  return (
    <div className="grid lg:grid-cols-2 gap-4">
      <Card className="p-4">
        <h3 className="font-semibold text-[#002443] mb-3">Eventos 2FA</h3>
        <p className="text-xs text-[#002443]/60 mb-4">Enrollments, TOTP, PINs e códigos de backup dos administradores.</p>
        <div className="space-y-2 max-h-[480px] overflow-y-auto">
          {(!twoFactor || twoFactor.length === 0) && <div className="text-center py-6 text-[#002443]/50 text-sm">Sem registros</div>}
          {twoFactor?.slice(0, 80).map(t => (
            <div key={t.id} className="flex items-center justify-between gap-2 text-xs border-b border-slate-100 pb-2">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[#002443]/50">{format(new Date(t.created_date), 'dd/MM/yy HH:mm')}</div>
              </div>
      ...
      <Card className="p-4">
        <h3 className="font-semibold text-[#002443] mb-3">Tentativas de Login Admin</h3>
        <p className="text-xs text-[#002443]/60 mb-4">Logs de cada tentativa de login no portal administrativo.</p>
        <div className="space-y-2 max-h-[480px] overflow-y-auto">
          {(!loginAttempts || loginAttempts.length === 0) && <div className="text-center py-6 text-[#002443]/50 text-sm">Sem registros</div>}
          {loginAttempts?.slice(0, 80).map(l => (
            <div key={l.id} className="flex items-center justify-between gap-2 text-xs border-b border-slate-100 pb-2">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[#002443]/50">{format(new Date(l.created_date), 'dd/MM/yy HH:mm')} · {l.reason || '—'}</div>
              </div>
              {l.success
