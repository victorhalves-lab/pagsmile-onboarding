...
export default function AccessAuditTable({ items }) {
  return (
    <Card className="p-4">
      <h3 className="font-semibold text-[#0A0A0A] mb-3">Auditoria de Acessos & Permissões</h3>
      <p className="text-xs text-[#0A0A0A]/60 mb-4">Registra cada tentativa de acesso a páginas e ações sensíveis, indicando se foi permitida ou bloqueada pelo perfil.</p>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {(!items || items.length === 0) && <tr><td colSpan={6} className="text-center py-8 text-[#0A0A0A]/50">Sem registros</td></tr>}
            {items?.slice(0, 150).map(a => (
              <tr key={a.id} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 text-xs text-[#0A0A0A]/70">{a.target_page || a.target_entity || '—'}</td>
