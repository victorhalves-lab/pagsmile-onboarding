...
export function StepBlock({ number, title, description, condition, children }) {
  return (
    <div className="bg-white border border-[#002443]/10 rounded-2xl overflow-hidden">
      <div className="bg-[#002443] text-white px-5 py-3 flex items-center gap-3">
        <div className="w-7 h-7 rounded-full bg-[#2bc196] text-white text-xs font-bold flex items-center justify-center flex-shrink-0">
          {number}
        </div>
...
export function Question({ id, label, type, required, options, hint, condition }) {
  return (
    <div className="border-l-2 border-[#2bc196]/40 pl-4 py-1">
      <div className="flex items-start gap-2 flex-wrap">
        {id && (
          <code className="text-[10px] bg-[#002443]/5 text-[#002443]/70 px-1.5 py-0.5 rounded font-mono">
            {id}
          </code>
        )}
        ...
        {type && (
          <span className="text-[10px] bg-[#2bc196]/10 text-[#2bc196] px-1.5 py-0.5 rounded font-bold uppercase">
            {type}
          </span>
        )}
      </div>
      {hint && <p className="text-[11px] text-[#002443]/60 mt-1 italic">{hint}</p>}
      {condition && (
      ...
      {options && options.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {options.map((opt, i) => (
            <span
              key={i}
              className="text-[11px] bg-[#f4f4f4] text-[#002443] px-2 py-0.5 rounded border border-[#002443]/10"
            >
              {opt}
            </span>
          ))}
...
export function FlagCard({ code, name, description, penalty }) {
  return (
    <div className="bg-white border border-[#002443]/10 rounded-lg p-3 flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs font-bold text-[#002443]">{name}</div>
        <div className="text-[11px] text-[#002443]/70 mt-0.5">{description}</div>
        {penalty && (
...
export function ScoreRule({ condition, points, type = 'bonus' }) {
  ...
  return (
    <div className={`flex items-center gap-3 p-2 rounded ${isBonus ? 'bg-emerald-50' : 'bg-red-50'}`}>
      ...
      <span className="text-xs text-[#002443]">{condition}</span>
    </div>
  );
}
