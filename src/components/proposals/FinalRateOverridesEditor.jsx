...
export default function FinalRateOverridesEditor({ overrides = {}, onChange, hideCalculationColumns = false, onToggleHideColumns, hideRange13a21 = false, onToggleHideRange13a21 }) {
  ...
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/5 p-5 space-y-4">
      ...
      {/* Opção de ocultar faixa 13x-21x */}
      {onToggleHideRange13a21 && (
        <label className="flex items-center gap-2.5 bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2.5 cursor-pointer hover:bg-white/[0.05] transition-colors">
          <input
            type="checkbox"
            checked={hideRange13a21}
            onChange={(e) => onToggleHideRange13a21(e.target.checked)}
            className="w-4 h-4 rounded border-white/20 text-[#2bc196] focus:ring-[#2bc196] bg-white/5"
          />
          <div>
            <span className="text-xs font-medium text-white">Ocultar faixa 13x a 21x</span>
            <p className="text-[10px] text-white/30">Remove as parcelas de 13x a 21x da proposta do cliente</p>
          </div>
        </label>
      )}
