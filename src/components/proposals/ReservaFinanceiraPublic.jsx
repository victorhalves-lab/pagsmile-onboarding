...
export default function ReservaFinanceiraPublic({ rates, segmentName }) {
  ...
  return (
    <Card className="mb-6 border-[#1356E2]/20">
      <CardContent className="py-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <h2 className="font-bold text-base text-[#0A0A0A]">Reserva Financeira</h2>
            <p className="text-xs text-[#0A0A0A]/50">Rolling Reserve — proteção contra chargebacks e disputas</p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
          {pixAtiva && (
            <div className="rounded-xl bg-[#1356E2]/5 border border-[#1356E2]/20 p-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-bold text-[#0A0A0A]">PIX</p>
                <span className="text-[10px] font-semibold text-[#0A0A0A]/40 uppercase tracking-wider">
                  {reserva.pix.diasRetencao} dias
                </span>
              </div>
              <p className="text-3xl font-extrabold text-[#1356E2]">
                {reserva.pix.percentual}<span className="text-base text-[#1356E2]/60">%</span>
              </p>
              <p className="text-[11px] text-[#0A0A0A]/50 mt-1">retidos por {reserva.pix.diasRetencao} dias</p>
            </div>
          )}
          {cartaoAtiva && (
            <div className="rounded-xl bg-[#0A0A0A]/5 border border-[#0A0A0A]/10 p-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-bold text-[#0A0A0A]">Cartão</p>
                <span className="text-[10px] font-semibold text-[#0A0A0A]/40 uppercase tracking-wider">
                  {reserva.cartao.diasRetencao} dias
                </span>
              </div>
              <p className="text-3xl font-extrabold text-[#0A0A0A]">
                {reserva.cartao.percentual}<span className="text-base text-[#0A0A0A]/40">%</span>
              </p>
              <p className="text-[11px] text-[#0A0A0A]/50 mt-1">retidos por {reserva.cartao.diasRetencao} dias</p>
            </div>
          )}
        </div>
