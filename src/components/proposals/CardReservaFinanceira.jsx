...
import {
  ...
  DEFAULT_RESERVA, getReservaWithDefaults,
  ...
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/5 p-5 space-y-4">
      ...
      <div className="space-y-2">
        <Label className={labelCls}>Disclaimer exibido ao cliente</Label>
        <textarea
          value={reserva.disclaimer}
          onChange={(e) => !readOnly && update('disclaimer', e.target.value)}
          disabled={readOnly}
          rows={2}
          className={`w-full bg-white/5 border border-white/10 text-white text-xs rounded-xl p-3 placeholder:text-white/15 focus:border-[#2bc196] focus:ring-1 focus:ring-[#2bc196] outline-none resize-none ${readOnly ? 'opacity-60 cursor-not-allowed' : ''}`}
        />
        <div className="flex items-start gap-1.5 text-[10px] text-white/40">
          <Info className="w-3 h-3 mt-0.5 flex-shrink-0" />
          <span>Esse texto aparece na proposta pública ao lado dos percentuais de reserva.</span>
        </div>
      </div>
    </div>
  );
