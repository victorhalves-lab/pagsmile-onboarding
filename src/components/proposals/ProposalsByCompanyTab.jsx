    ...
    <div className="space-y-4">
      ...
      {grouped.length === 0 && (
        <div className="text-center py-12">
          <Building2 className="w-12 h-12 mx-auto text-[#002443]/20 mb-3" />
      ...
      <div className="space-y-2">
        {grouped.map(({ company, proposals, total }) => {
          ...
          return (
            <div key={company} className="bg-white rounded-xl border border-[#002443]/5 shadow-sm overflow-hidden">
              <button
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-[#002443] truncate">{company}</p>
                  <p className="text-xs text-[#002443]/50">{total} proposta{total > 1 ? 's' : ''}</p>
                </div>
                {isExpanded ? <ChevronDown className="w-4 h-4 text-[#002443]/40" /> : <ChevronRight className="w-4 h-4 text-[#002443]/40" />}
              </button>
                  ...
                  {proposals.map(p => {
                    ...
                    return (
                      <div key={p.id} className="flex items-center gap-4 px-5 py-3 hover:bg-[#f4f4f4]/50 transition-colors">
                        <FileText className="w-4 h-4 text-[#002443]/30 flex-shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-[#002443] truncate">
                            <span className="font-mono text-[#2bc196] mr-2">{p.codigo || '—'}</span>
                            {p.proposalName || ''}
                          </p>
                          <p className="text-xs text-[#002443]/50">
                            Criada em {moment(p.created_date).format('DD/MM/YYYY')}
                            {p.version > 1 && <span className="ml-2 text-[#2bc196]">v{p.version}</span>}
                          </p>
                        </div>
