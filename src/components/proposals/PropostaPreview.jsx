  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[10px] text-[#FFB81C]/50 font-mono">{form.clienteNome || '—'}</span>
      </div>
        ...
        {visibleRows.map(row => (
          <div key={row.parcela} className="grid grid-cols-5 items-center px-2 py-[5px] border-t border-white/[0.03]">
            ...
            <div className="text-[10px] text-right font-mono">
              {row.taxaAntecipacao > 0 ? <span className="text-[#FFB81C]">+{fmtPct(row.taxaAntecipacao)}</span> : <span className="text-white/30">—</span>}
            </div>
            <div className={`text-[11px] text-right font-bold ${row.hasOverride ? 'text-amber-400' : 'text-[#FFB81C]'}`}>{fmtPct(row.taxaFinal)}</div>
          </div>
        ))}
      </div>
      ...
      {usaMaquininha && (
        <div className="rounded-xl bg-[#1356E2]/[0.04] border border-[#1356E2]/10 overflow-hidden mb-4">
          <div className="px-2 py-1.5 bg-[#1356E2]/[0.06] flex items-center justify-between">
            <span className="text-[9px] text-[#FFB81C] font-bold uppercase tracking-widest">📱 Maquininha (Presencial)</span>
          ...
          <div className="grid grid-cols-4 items-center px-2 py-1.5 border-t border-white/[0.03]">
            <div className="text-[11px] text-center font-bold text-[#FFB81C]">{fmtPct(mqCredito.avista)}</div>
            <div className="text-[11px] text-center font-bold text-[#FFB81C]">{fmtPct(mqCredito.de2a6x)}</div>
            <div className="text-[11px] text-center font-bold text-[#FFB81C]">{fmtPct(mqCredito.de7a12x)}</div>
            <div className="text-[11px] text-center font-bold text-[#FFB81C]">{fmtPct(mqDebito)}</div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: 'Prazo', value: prazo, color: 'text-[#FFB81C]' },
            { label: 'Antecipação', value: `${fmtPct(taxaRAV)} a.m.`, color: 'text-white' },
            { label: 'PIX', value: rates.pix?.tipo === 'fixo' ? `R$ ${parseVal(rates.pix?.valor).toLocaleString('pt-BR', {minimumFractionDigits: 2})}` : fmtPct(rates.pix?.valor), color: 'text-[#FFB81C]' },
        ...
        <div className="bg-white/[0.03] border border-white/5 rounded-xl p-2">
          <p className="text-[8px] text-[#FFB81C] font-bold uppercase tracking-widest text-center mb-1">TPV Mínimo Mensal Garantido</p>
          <div className="flex items-center justify-center gap-2 text-[9px]">
            ...
            <span className="text-white/40">→</span>
            <span className="text-[#FFB81C] font-bold">Mês 3 em diante: R$ {parseVal(rates.minimoGarantido?.mes3).toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span>
          </div>
        </div>
      </div>
    </div>
