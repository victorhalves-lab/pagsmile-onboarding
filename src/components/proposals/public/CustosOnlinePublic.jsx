  ...
  return (
    <Card className="mb-4">
      <CardContent className="py-4">
        <h3 className="font-bold text-sm text-[#002443] mb-1 uppercase tracking-wide">Custos Adicionais por Transação</h3>
        <p className="text-[11px] text-[#002443]/50 mb-4">Aplicados em cada venda processada online (e-commerce, link de pagamento, checkout)</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {items.map(({ key, label, value, icon: Icon }) => (
            <div key={key} className="bg-[#f4f4f4] rounded-xl p-3 border border-[#2bc196]/10">
              <div className="flex items-center gap-1.5 mb-1.5">
                <Icon className="w-3.5 h-3.5 text-[#36706c]" />
                <p className="text-[10px] text-[#002443]/60 uppercase font-semibold tracking-wide">{label}</p>
              </div>
              <p className="text-base font-bold text-[#002443] font-mono">{value}</p>
            </div>
          ))}
        </div>
      </CardContent>
