    ...
    <div className="rounded-2xl bg-white/[0.03] border border-white/5 p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          ...
          <TaxaInput value={form.taxaAntecipacao || ''} onChange={(val) => onUpdate('taxaAntecipacao', val)} placeholder="0,00" suffix="a.m." className={`${inputCls} text-right pr-14`} />
        </div>
        <div className="space-y-1.5">
          <Label className={labelCls}>Prazo de Recebimento</Label>
          <div className="grid grid-cols-3 gap-1.5">
            {PRAZOS.map(p => (
              <button key={p.value} onClick={() => onUpdate('prazoRecebimento', p.value)}
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-white/5">
        <Switch id="usa-antecipacao" checked={form.usaAntecipacao} onCheckedChange={(checked) => onUpdate('usaAntecipacao', checked)}
      ...
      {form.usaAntecipacao && (
        <div className="space-y-1.5">
          ...
          <TaxaInput value={form.percentualAntecipacao || ''} onChange={(val) => onUpdate('percentualAntecipacao', val)} placeholder="100,00" suffix="%" className={`${inputCls} text-right pr-10`} />
        </div>
      )}
    </div>
  );
