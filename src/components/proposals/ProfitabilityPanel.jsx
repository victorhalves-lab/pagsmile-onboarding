...
export default function ProfitabilityPanel({ rates, form, partner, leadTpv, leadTransacoes, selectedMccCode }) {
      ...
      <div className="rounded-2xl bg-white/[0.03] border border-white/5 p-5">
        <div className="flex items-center gap-2 mb-3">
          <div className="w-7 h-7 rounded-lg bg-[#1356E2]/10 flex items-center justify-center">
            <TrendingUp className="w-3.5 h-3.5 text-[#FEA500]" />
          </div>
  ...
  const marginColor = isPositive ? 'text-[#FEA500]' : profitability?.margem < 0 ? 'text-red-400' : 'text-white/50';
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-[#1356E2]/5 border border-[#1356E2]/10 rounded-xl p-3 text-center">
              <p className="text-[8px] text-[#FEA500]/70 font-bold uppercase tracking-wider">Receita Est.</p>
              <p className="text-sm font-bold text-[#FEA500]">{fmtBRL(profitability.receitaTotal)}</p>
            </div>
          ...
          <div className="rounded-xl bg-white/[0.02] border border-white/5 overflow-hidden">
            ...
            {[
              ...
              <div key={row.label} className="grid grid-cols-4 items-center px-3 py-2 border-t border-white/[0.03]">
                ...
                <div className="text-[10px] text-right text-[#FEA500] font-mono">{fmtBRL(row.receita)}</div>
                <div className="text-[10px] text-right text-red-400 font-mono">{fmtBRL(row.custo)}</div>
                <div className={`text-[10px] text-right font-bold font-mono ${row.margem >= 0 ? 'text-[#FEA500]' : 'text-red-400'}`}>{fmtBRL(row.margem)}</div>
              </div>
            ))}
            <div className="grid grid-cols-4 items-center px-3 py-2 border-t border-white/10 bg-white/[0.02]">
              ...
              <div className="text-[10px] text-right text-[#FEA500] font-bold font-mono">{fmtBRL(profitability.receitaTotal)}</div>
              <div className="text-[10px] text-right text-red-400 font-bold font-mono">{fmtBRL(profitability.custoTotal)}</div>
              <div className={`text-[10px] text-right font-bold font-mono ${profitability.margem >= 0 ? 'text-[#FEA500]' : 'text-red-400'}`}>{fmtBRL(profitability.margem)}</div>
            </div>
          </div>
          ...
          {profitability.alertas.length === 0 && (
            <div className="rounded-xl bg-[#1356E2]/5 border border-[#1356E2]/10 p-2.5 flex items-center gap-2">
              <CheckCircle2 className="w-3.5 h-3.5 text-[#FEA500]" />
              <p className="text-[9px] text-[#FEA500] font-medium">Taxas dentro dos limites do parceiro</p>
            </div>
          )}
