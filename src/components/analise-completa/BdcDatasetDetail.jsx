    ...
    <div className="border border-slate-200 rounded-lg overflow-hidden">
      <div className="p-3 bg-slate-50/50">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="text-[var(--pinbank-blue)]/40">
              {date.toLocaleDateString('pt-BR')} {date.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
            </span>
            {record.duration_ms && <span className="text-[var(--pinbank-blue)]/40">• {record.duration_ms}ms</span>}
            {record.responseTime && <span className="text-[var(--pinbank-blue)]/40">• {record.responseTime}ms</span>}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
            {readableFields.map(({ key, value, isNested }, i) => (
              <div key={i} className={`text-xs p-2 rounded bg-white border border-slate-100 ${isNested ? 'col-span-full' : ''}`}>
                <span className="text-[var(--pinbank-blue)]/40 text-[10px] font-semibold">{humanizeKey(key)}: </span>
                {isNested ? (
                  <pre className="text-[10px] text-[var(--pinbank-blue)]/70 mt-1 whitespace-pre-wrap font-mono">{JSON.stringify(value, null, 2)}</pre>
                ) : (
                  <span className="text-[var(--pinbank-blue)] font-medium">{String(value)}</span>
                )}
              </div>
            ))}
          </div>
      ...
      {hasData && (
        <div className="px-3 pb-3">
          <button onClick={() => setShowRaw(!showRaw)} className="text-[10px] text-[var(--pinbank-blue)]/40 hover:text-[var(--pinbank-blue)]/60 flex items-center gap-1 mt-2">
