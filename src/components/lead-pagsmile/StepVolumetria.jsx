...
export default function StepVolumetria({ form, updateField, errors }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-bold text-[#0A0A0A]">Volumetria e Faturamento</h2>
      </div>
      ...
      {(() => {
        ...
        return (
          <div className="space-y-1" data-field="tpvMensal">
            <label className="text-sm font-semibold text-[#0A0A0A]">TPV Mensal (R$) *</label>
            <CurrencyNumberInput
            ...
            <p className="text-[10px] text-[#0A0A0A]/40">Volume total de pagamentos processados por mês</p>
            {tooLow && (
      ...
      <div className="space-y-2" data-field="faturamentoAnual">
        <label className="text-sm font-semibold text-[#0A0A0A]">Faturamento Anual da Empresa *</label>
      ...
      <div className="space-y-2" data-field="funcionarios">
        <label className="text-sm font-semibold text-[#0A0A0A]">Número de Funcionários *</label>
      ...
      <div className="space-y-1" data-field="ticketMedio">
        <label className="text-sm font-semibold text-[#0A0A0A]">Ticket Médio (R$) *</label>
      ...
      <div className="space-y-1">
        <label className="text-sm font-semibold text-[#0A0A0A]">Transações/Mês</label>
        <div className="h-12 rounded-xl bg-[#f4f4f4] border border-[#0A0A0A]/5 flex items-center px-4">
          <span className="text-sm font-mono font-semibold text-[#0A0A0A]">
            {form.transacoesMes ? Number(form.transacoesMes).toLocaleString('pt-BR') : '—'}
          </span>
          <span className="text-[10px] text-[#0A0A0A]/40 ml-2">Calculado automaticamente (TPV ÷ Ticket Médio)</span>
        </div>
      </div>
    </div>
  );
