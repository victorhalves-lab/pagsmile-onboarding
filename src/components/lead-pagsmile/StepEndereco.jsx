  ...
  if (!hasAddress && !editMode) {
    return (
      <div className="space-y-5">
        <div>
          <h2 className="text-lg font-bold text-[#0A0A0A]">Endereço da Empresa</h2>
          <p className="text-xs text-[#0A0A0A]/50 mt-1">Não foi possível obter o endereço automaticamente. Você pode informar manualmente ou avançar.</p>
        </div>
        ...
        <p className="text-[10px] text-[#1356E2] font-medium">✓ Você pode avançar sem endereço</p>
      </div>
    );
  }
      ...
      {!editMode ? (
        ...
        <div className="space-y-4">
          <div className={`rounded-xl border-2 p-5 transition-all ${
            confirmed ? 'border-[#1356E2] bg-[#1356E2]/5' : 'border-[#0A0A0A]/10 bg-[#f4f4f4]'
          }`}>
            <div className="flex items-start gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                confirmed ? 'bg-[#1356E2]/20' : 'bg-[#0A0A0A]/5'
              }`}>
                <MapPin className={`w-5 h-5 ${confirmed ? 'text-[#1356E2]' : 'text-[#0A0A0A]/40'}`} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#0A0A0A]">{formatEnderecoLine() || 'Endereço não disponível'}</p>
                <p className="text-sm text-[#0A0A0A]/60 mt-0.5">{formatCidadeLine()}</p>
                {displayEndereco.cep && (
                  <p className="text-xs text-[#0A0A0A]/40 mt-1 font-mono">CEP: {formatCep(displayEndereco.cep)}</p>
                )}
              </div>
              {confirmed && <CheckCircle className="w-6 h-6 text-[#1356E2] shrink-0" />}
            </div>
          </div>
          ...
          <div className="flex gap-3">
            {!confirmed ? (
              <>
                <Button onClick={handleConfirm} className="bg-[#1356E2] hover:bg-[#1356E2]/90 text-white rounded-xl gap-2 flex-1">
          ...
          {confirmed && (
            <p className="text-[10px] text-[#1356E2] font-medium">✓ Endereço confirmado</p>
          )}
        </div>
      ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-sm font-semibold text-[#0A0A0A]">CEP *</label>
          ...
          {/* Logradouro */}
          <div className="space-y-1">
            <label className="text-sm font-semibold text-[#0A0A0A]">Logradouro</label>
            <Input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-sm font-semibold text-[#0A0A0A]">Número</label>
              <Input
            ...
            <div className="space-y-1">
              <label className="text-sm font-semibold text-[#0A0A0A]">Complemento</label>
              <Input
          ...
          {/* Bairro */}
          <div className="space-y-1">
            <label className="text-sm font-semibold text-[#0A0A0A]">Bairro</label>
            <Input
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2 space-y-1">
              <label className="text-sm font-semibold text-[#0A0A0A]">Cidade</label>
              <Input
            ...
            <div className="space-y-1">
              <label className="text-sm font-semibold text-[#0A0A0A]">UF</label>
              <Input
          ...
          {/* Botão confirmar */}
          <Button onClick={handleConfirm} className="w-full bg-[#1356E2] hover:bg-[#1356E2]/90 text-white rounded-xl gap-2 h-12">
            <CheckCircle className="w-4 h-4" /> Confirmar Endereço
          </Button>
        </div>
      )}
