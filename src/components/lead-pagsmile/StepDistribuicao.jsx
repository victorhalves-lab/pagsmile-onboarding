...
export default function StepDistribuicao({ form, updateField, errors = {} }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-bold text-[#0A0A0A]">Distribuição de Meios de Pagamento</h2>
      </div>

      {/* P18 — Já processa? */}
      <div className="space-y-2" data-field="jaProcessa">
        <label className="text-sm font-semibold text-[#0A0A0A]">Já processa pagamentos? *</label>
      ...
      {jaProcessa && (
        <div className="space-y-2" data-field="distribuicao">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <label className="text-sm font-semibold text-[#0A0A0A]">Distribuição atual por meio de pagamento (%) *</label>
            <span className={`text-xs font-bold ${totalDist === 100 ? 'text-[#1356E2]' : 'text-red-600'}`}>
              Soma: {totalDist}% {totalDist === 100 ? '✓' : `(faltam ${100 - totalDist}%)`}
            </span>
          </div>
          <p className="text-xs text-[#0A0A0A]/50">Os 4 sliders devem somar 100%</p>
          <SliderDistribution
      ...
      {form.jaProcessa === 'Não, estou começando' && (
        <div className="space-y-2" data-field="distribuicaoDesejada">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <label className="text-sm font-semibold text-[#0A0A0A]">Distribuição desejada por meio de pagamento (%) *</label>
            <span className={`text-xs font-bold ${totalDes === 100 ? 'text-[#1356E2]' : 'text-red-600'}`}>
              Soma: {totalDes}% {totalDes === 100 ? '✓' : `(faltam ${100 - totalDes}%)`}
            </span>
          </div>
          <p className="text-xs text-[#0A0A0A]/50">Como você gostaria de distribuir seus pagamentos?</p>
          <SliderDistribution
      ...
      {form.jaProcessa && (
        <div className="space-y-2" data-field="distribuicaoParcelamento">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            <span className={`text-xs font-bold ${totalParc === 100 ? 'text-[#1356E2]' : 'text-red-600'}`}>
      ...
      {jaProcessa && (
        <div className="space-y-2">
          <label className="text-sm font-semibold text-[#0A0A0A]">Já usa antecipação?</label>
