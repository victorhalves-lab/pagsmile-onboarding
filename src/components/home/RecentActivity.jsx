...
const ACTION_CONFIG = {
  CREATE: { icon: Plus, color: 'bg-[#1356E2]/10 text-[#1356E2]' },
  UPDATE: { icon: Pencil, color: 'bg-[#E84B1C]/10 text-[#E84B1C]' },
  DELETE: { icon: Trash2, color: 'bg-red-100 text-red-500' },
  VIEW: { icon: Eye, color: 'bg-[#0A0A0A]/10 text-[#0A0A0A]' },
  APPROVAL: { icon: CheckCircle, color: 'bg-[#1356E2]/10 text-[#1356E2]' },
  REJECTION: { icon: XCircle, color: 'bg-red-100 text-red-500' },
  VALIDATION: { icon: Shield, color: 'bg-[#E84B1C]/10 text-[#E84B1C]' },
};
  ...
  if (!logs || logs.length === 0) {
    return (
      <Card className="rounded-2xl border-[#0A0A0A]/5 shadow-sm">
        ...
        <CardContent>
          <p className="text-sm text-[#0A0A0A]/30 text-center py-6">{t('activity.none')}</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <CardContent>
        <div className="space-y-1">
          {logs.map((log, idx) => {
            ...
            return (
              <div key={log.id || idx} className="flex items-start gap-3 p-3 rounded-xl hover:bg-[#F7F5F0] transition-colors">
                <div className={`p-2 rounded-lg ${config.color} shrink-0 mt-0.5`}>
                  <Icon className="w-3.5 h-3.5" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-[#0A0A0A] font-medium leading-relaxed line-clamp-2">
                    {log.actionDescription || `${log.actionType} em ${log.entityName}`}
                  </p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-[#0A0A0A]/40 font-semibold">
                      {log.changedBy?.split('@')[0] || t('activity.system')}
                    </span>
                    <span className="text-[10px] text-[#0A0A0A]/20">•</span>
