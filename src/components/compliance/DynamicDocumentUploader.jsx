...
// Card especial para selfie com documento
function SelfieCard({ uploadedFile, onUpload, onRemove, isUploading }) {
  const inputRef = React.useRef(null);
  const isUploaded = !!uploadedFile?.url;
  ...
  const handleFileSelect = (e) => {
    ...
    if (!['JPG', 'JPEG', 'PNG'].includes(ext)) {
      toast.error('Use formatos JPG ou PNG para a selfie.');
      return;
    }
    onUpload(file);
    e.target.value = '';
  ...
  return (
    <div className={`rounded-xl border-2 p-5 transition-all duration-200 ${
      isUploaded
        ? 'border-green-200 bg-green-50/50'
        : 'border-amber-300 bg-gradient-to-br from-amber-50/80 to-orange-50/40'
    }`}>
      <div className="flex items-start gap-4">
        <div className={`p-3 rounded-xl shrink-0 ${isUploaded ? 'bg-green-100' : 'bg-amber-100'}`}>
          {isUploaded
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-bold text-[#002443] text-sm mb-1">
            Selfie com Documento <span className="text-red-500">*</span>
          </h4>
          {!isUploaded ? (
            <>
              <p className="text-xs text-[#002443]/60 mb-3 leading-relaxed">
                Tire uma foto segurando seu <strong>RG ou CNH</strong> ao lado do rosto. 
                A imagem deve mostrar claramente seu rosto e o documento legível.
              </p>
              <div className="bg-white/80 rounded-lg p-3 mb-3 border border-amber-200/60">
                <p className="text-xs font-semibold text-[#002443]/70 mb-2">Como tirar a foto:</p>
                <ul className="text-xs text-[#002443]/60 space-y-1.5">
                  <li className="flex items-start gap-2">
                    <User className="w-3.5 h-3.5 text-amber-500 mt-0.5 shrink-0" />
                    Posicione seu rosto centralizado na imagem
                  </li>
                  <li className="flex items-start gap-2">
                    <Camera className="w-3.5 h-3.5 text-amber-500 mt-0.5 shrink-0" />
                    Segure o documento (RG ou CNH) ao lado do rosto
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-3.5 h-3.5 text-amber-500 mt-0.5 shrink-0" />
                    Garanta boa iluminação e que o documento esteja legível
                  </li>
                </ul>
              </div>
              <input
              ...
              <Button
                ...
                onClick={() => inputRef.current?.click()}
                disabled={isUploading}
                className="h-9 text-xs border-amber-300 hover:bg-amber-50"
              >
                {isUploading
                  ? <Loader2 className="w-3 h-3 mr-2 animate-spin" />
                  : <Upload className="w-3 h-3 mr-2" />
