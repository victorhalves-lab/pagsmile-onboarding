  ...
  return (
    <div className="space-y-4">
      <Tabs value={subTab} onValueChange={setSubTab}>
        <TabsList className="bg-white border border-[#0A0A0A]/10 p-1 rounded-xl mb-4">
          <TabsTrigger value="pj" className="rounded-lg data-[state=active]:bg-[#0A0A0A] data-[state=active]:text-white gap-2 px-4">
            <Building2 className="w-4 h-4" /> Pessoa Jurídica
            <Badge className="bg-[#1356E2]/20 text-[#0A0A0A] text-xs ml-1 border-0">{pjCases.length}</Badge>
          </TabsTrigger>
          <TabsTrigger value="pf" className="rounded-lg data-[state=active]:bg-[#0A0A0A] data-[state=active]:text-white gap-2 px-4">
      ...
      <div className="text-sm text-[var(--pinbank-blue)]/50 mb-2">
      ...
      {sellerGroups.length === 0 ? (
        <Card><CardContent className="text-center py-12">
          <Users className="w-12 h-12 mx-auto text-[var(--pinbank-blue)]/20 mb-4" />
          <p className="text-[var(--pinbank-blue)]/50">Nenhum questionário de subseller recebido ainda.</p>
        </CardContent></Card>
      ) : (
              ...
              <Card key={group.sellerId} className="overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-[var(--pinbank-blue)]">{group.sellerName}</p>
                      <p className="text-xs text-[var(--pinbank-blue)]/50">{group.sellerCnpj}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    {isExpanded ? <ChevronDown className="w-5 h-5 text-[var(--pinbank-blue)]/40" /> : <ChevronRight className="w-5 h-5 text-[var(--pinbank-blue)]/40" />}
                  </div>
                </button>

                {isExpanded && (
                  <div className="border-t border-[var(--pinbank-blue)]/5">
                    <Table>
                      <TableHeader>
                            ...
                            <TableRow key={c.id} className="hover:bg-[#f4f4f4]/50">
                              <TableCell>
                                <div className="flex items-center gap-2">
                                  <div>
                                    <p className="font-medium text-[var(--pinbank-blue)]">{m?.fullName || 'N/A'}</p>
                                    <p className="text-xs text-[var(--pinbank-blue)]/50">{m?.cpfCnpj || '-'}</p>
                                  </div>
                              ...
                              <TableCell>
                                {score?.segmento ? (
                                  <Badge className="bg-[var(--pinbank-blue)]/5 text-[var(--pinbank-blue)] border-0 text-xs capitalize">{score.segmento}</Badge>
                              ...
                              <TableCell className="text-center">
                                ...
                                ) : <span className="text-[var(--pinbank-blue)]/40">-</span>}
                              </TableCell>
                              <TableCell className="text-center">
                              ...
                              <TableCell>
                                <span className="text-xs text-[var(--pinbank-blue)]/70">
