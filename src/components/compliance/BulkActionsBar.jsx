...
export default function BulkActionsBar({ selectedRows, setSelectedRows, casesMap, merchantMap, onRefresh }) {
    ...
    for (const link of withEmail) {
      try {
        await base44.integrations.Core.SendEmail({
          ...
          body: `<p>Olá <strong>${link.name}</strong>,</p><p>Para dar continuidade ao seu processo de onboarding, precisamos que envie os documentos solicitados e complete a verificação de identidade.</p><p><a href="${link.url}" style="display:inline-block;padding:12px 24px;background:#2bc196;color:white;text-decoration:none;border-radius:8px;font-weight:600;">Enviar Documentos e Verificar Identidade</a></p><p>Se o botão não funcionar, copie e cole o link abaixo:</p><p>${link.url}</p><p>Atenciosamente,<br/>Equipe Pagsmile</p>`,
        });
        sent++;
      } catch (err) {
  ...
  return (
    <>
      {/* Barra de ações */}
      <div className="bg-[#002443] rounded-xl p-3 flex items-center justify-between shadow-lg animate-in slide-in-from-top-2 duration-300">
        <div className="flex items-center gap-3">
          ...
          <Button
            size="sm"
            className="bg-[#2bc196] hover:bg-[#2bc196]/90 text-white rounded-lg text-xs gap-1.5"
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 py-2">
            {generatingLinks ? (
              <div className="flex items-center justify-center py-8 gap-3">
                ...
                <span className="text-sm text-[#002443]/70">Gerando links e resetando flags...</span>
              </div>
            ) : (
              generatedLinks.map((link, i) => (
                <div key={i} className={`p-3 rounded-lg border ${link.error ? 'bg-red-50 border-red-200' : 'bg-white border-[#002443]/8'}`}>
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-[#002443] truncate">{link.name}</p>
                      <p className="text-xs text-[#002443]/50">{link.email || 'Sem e-mail'}</p>
                    </div>
                    {link.error ? (
                  ...
                  {link.url && (
                    <p className="text-[10px] text-[#002443]/40 mt-1 font-mono truncate">{link.url}</p>
                  )}
                </div>
              ))
            )}
          ...
          {!generatingLinks && generatedLinks.length > 0 && (
            <DialogFooter className="flex-col sm:flex-row gap-2 border-t pt-4">
              ...
              <Button
                className="bg-[#2bc196] hover:bg-[#2bc196]/90 text-white gap-1.5 text-xs"
