...
const DistributionSlider = ({ label, value, onChange, color }) => (
  <div className="space-y-2">
    <div className="flex justify-between items-center">
      <Label className="text-sm font-medium text-[#002443]">{label}</Label>
    ...
    <div className="space-y-8">
      <div>
        <div className="flex items-center gap-3 mb-2">
          <BarChart3 className="w-5 h-5 text-[#002443]/50" />
          <h3 className="text-lg font-semibold text-[#002443]">Volume e Meios de Pagamento</h3>
        </div>
        <p className="text-sm text-[#002443]/70">Nos ajude a entender sua volumetria para oferecermos a melhor proposta.</p>
      </div>

      <div className="space-y-2">
        <Label className="text-sm font-semibold text-[#002443]">Qual seu TPV (Volume Total de Pagamentos) mensal estimado? <span className="text-red-500">*</span></Label>
        <CurrencyInput
            value={tpvMensal}
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
             <Percent className="w-4 h-4 text-[#002443]/50" />
            <Label className="text-sm font-semibold text-[#002443]">Distribuição do TPV por método <span className="text-red-500">*</span></Label>
            {totalDist !== 100 && <span className="text-xs font-bold text-red-500">(Soma deve ser 100%)</span>}
        </div>
        <div className="p-6 bg-white rounded-2xl border border-[#002443]/[0.08] space-y-5 shadow-sm">
            <DistributionSlider label="Cartão de Crédito" value={dist.cartao} onChange={v => handleDistributionChange('cartao', v)} color="#3b82f6" />
            <DistributionSlider label="PIX" value={dist.pix} onChange={v => handleDistributionChange('pix', v)} color="#2bc196" />
