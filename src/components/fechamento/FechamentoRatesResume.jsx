...
export default function FechamentoRatesResume({ segmentRates, segmentName, partnerName }) {
  ...
  return (
    <div className="bg-[#0A0A0A] rounded-2xl p-6 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-48 h-48 bg-[#1356E2]/8 rounded-full blur-[60px]" />
      <div className="relative z-10">
        ...
        <h3 className="text-xl font-extrabold mb-3" style={{ color: '#1356E2' }}>{segmentName}</h3>
        {segmentName && (
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {[
            ...
            <div key={r.label} className="bg-white/[0.06] border border-white/[0.08] rounded-lg px-3 py-2 text-center min-w-[70px]">
              ...
              <p className="text-base font-extrabold" style={{ color: '#1356E2' }}>{fmtPct(r.value)}</p>
            </div>
          ))}
        </div>
        ...
        {percentualAntecipacao != null && (
          <div className="flex items-center gap-2 bg-[#1356E2]/10 border border-[#1356E2]/20 rounded-lg px-3 py-2 mb-4">
            <Repeat className="w-4 h-4" style={{ color: '#1356E2' }} />
            <span className="text-xs font-semibold" style={{ color: 'rgba(255,255,255,0.6)' }}>Antecipação</span>
            <span className="ml-auto text-base font-extrabold" style={{ color: '#1356E2' }}>{fmtPct(percentualAntecipacao)} a.m.</span>
          </div>
        )}

        {/* PIX */}
        <div className="flex items-center gap-2 mb-2">
          <Smartphone className="w-4 h-4" style={{ color: '#1356E2' }} />
        ...
        <div className="flex gap-4 mb-3">
          {pixTaxaPercentual != null && (
            <div className="bg-white/[0.06] border border-white/[0.08] rounded-lg px-3 py-2 text-center">
              ...
              <p className="text-base font-extrabold" style={{ color: '#1356E2' }}>{fmtPct(pixTaxaPercentual)}</p>
            </div>
          )}
          {pixTaxaFixa != null && (
            <div className="bg-white/[0.06] border border-white/[0.08] rounded-lg px-3 py-2 text-center">
              <p className="text-[10px] font-semibold" style={{ color: 'rgba(255,255,255,0.4)' }}>Fixo</p>
              <p className="text-base font-extrabold" style={{ color: '#1356E2' }}>{fmtBrl(pixTaxaFixa)}</p>
            </div>
          )}
        </div>
