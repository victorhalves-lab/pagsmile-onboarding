  ...
  return (
    <div className="bg-white rounded-xl border border-[var(--pinbank-blue)]/8 p-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-xs text-[var(--pinbank-blue)]/50">{title}</p>
          <p className="text-sm font-semibold text-[var(--pinbank-blue)]">{value || '—'}</p>
          {sub && <p className="text-[10px] text-[var(--pinbank-blue)]/40">{sub}</p>}
        </div>
      </div>
    </div>
  );
...
export default function CadastroOverviewTab({ merchant, latestCase, lead, latestProposal, latestContract, latestScore, documents, subsellers, allProposals = [], allContracts = [], allLeads = [], allCases = [] }) {
  ...
  return (
    <div className="space-y-6 mt-4">
      ...
      {latestScore && (
        <div className="bg-white rounded-xl border border-[var(--pinbank-blue)]/8 p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
            <div>
              <p className="text-2xl font-bold text-[var(--pinbank-blue)]">{latestScore.score_final ?? latestScore.score_geral_composto ?? '—'}</p>
              <p className="text-[10px] text-[var(--pinbank-blue)]/50">Score Final</p>
            </div>
            <div>
              <p className="text-lg font-bold text-[var(--pinbank-blue)]">{latestScore.subfaixa || '—'}</p>
              <p className="text-[10px] text-[var(--pinbank-blue)]/50">Subfaixa</p>
            </div>
            <div>
              <p className="text-lg font-bold text-[var(--pinbank-blue)]">{latestScore.subfaixa_nome || '—'}</p>
              <p className="text-[10px] text-[var(--pinbank-blue)]/50">Classificação</p>
            </div>
            <div>
              <p className="text-lg font-bold text-[var(--pinbank-blue)]">{latestScore.recomendacao_final || '—'}</p>
              <p className="text-[10px] text-[var(--pinbank-blue)]/50">Recomendação</p>
            </div>
            <div>
              <p className="text-lg font-bold text-[var(--pinbank-blue)]">{latestScore.monitoramento_nivel || '—'}</p>
              <p className="text-[10px] text-[var(--pinbank-blue)]/50">Monitoramento</p>
            </div>
          </div>
        </div>
      )}
      ...
      {lead && (
        <div className="bg-white rounded-xl border border-[var(--pinbank-blue)]/8 p-5">
          <h3 className="text-sm font-semibold text-[var(--pinbank-blue)] mb-3">Dados Comerciais (Lead)</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            {lead.tpvMensal && (
              <div>
                <p className="text-[10px] text-[var(--pinbank-blue)]/50">TPV Mensal</p>
                <p className="font-semibold">R$ {Number(lead.tpvMensal).toLocaleString('pt-BR')}</p>
              </div>
            )}
            {lead.ticketMedio && (
              <div>
                <p className="text-[10px] text-[var(--pinbank-blue)]/50">Ticket Médio</p>
                <p className="font-semibold">R$ {Number(lead.ticketMedio).toLocaleString('pt-BR')}</p>
              </div>
            )}
            {lead.businessSubCategory && (
              <div>
                <p className="text-[10px] text-[var(--pinbank-blue)]/50">Segmento</p>
                <p className="font-semibold">{segmentLabel(lead.businessSubCategory)}</p>
              </div>
            )}
            {lead.priscilaRiskLevel && (
              <div>
                <p className="text-[10px] text-[var(--pinbank-blue)]/50">Risco PRISCILA</p>
        ...
        <div className="bg-white rounded-xl border border-[var(--pinbank-blue)]/8 p-5">
          ...
          <div className="grid grid-cols-3 md:grid-cols-5 gap-3 text-center text-xs">
            <div>
              ...
              <p className="text-[var(--pinbank-blue)]/50">Aprovados</p>
            </div>
            <div>
              <p className="text-lg font-bold text-amber-600">{subsellers.filter(s => s.onboardingStatus === 'Manual').length}</p>
              <p className="text-[var(--pinbank-blue)]/50">Manual</p>
            </div>
            <div>
              <p className="text-lg font-bold text-blue-600">{subsellers.filter(s => ['Pendente', 'Em Análise'].includes(s.onboardingStatus)).length}</p>
              <p className="text-[var(--pinbank-blue)]/50">Pendentes</p>
            </div>
            <div>
              <p className="text-lg font-bold text-red-600">{subsellers.filter(s => s.onboardingStatus === 'Recusado').length}</p>
              <p className="text-[var(--pinbank-blue)]/50">Recusados</p>
            </div>
            <div>
              <p className="text-lg font-bold text-[var(--pinbank-blue)]">{subsellers.filter(s => s.type === 'PF').length}</p>
              <p className="text-[var(--pinbank-blue)]/50">PF</p>
            </div>
          </div>
        </div>
      )}
