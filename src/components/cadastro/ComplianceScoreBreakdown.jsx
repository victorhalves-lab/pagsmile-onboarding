  ...
  return (
    <div className="p-3 bg-gray-50 rounded-lg">
      <div className="flex justify-between text-xs mb-1.5">
        <span className="text-[var(--pagsmile-blue)]/60 font-medium">{label}</span>
      ...
      {description && <p className="text-[10px] text-[var(--pagsmile-blue)]/40 mt-1">{description}</p>}
    </div>
  );
}
...
export default function ComplianceScoreBreakdown({ score }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[var(--pagsmile-blue)]/8 p-5">
      <h3 className="text-sm font-semibold text-[var(--pagsmile-blue)] mb-1 flex items-center gap-2">
      ...
      {hasV4Breakdown && (
        <>
          <p className="text-xs text-[var(--pagsmile-blue)]/50 mb-4">
          ...
          {/* Visual Formula */}
          <div className="flex items-center gap-2 mb-4 p-3 bg-[var(--pagsmile-blue)]/5 rounded-lg overflow-x-auto">
            <div className="text-center flex-shrink-0">
              <p className="text-lg font-bold text-[var(--pagsmile-blue)]">{score.score_base_segmento ?? '?'}</p>
              <p className="text-[9px] text-[var(--pagsmile-blue)]/50">C1 Base</p>
            </div>
            <span className="text-lg text-[var(--pagsmile-blue)]/30">+</span>
            <div className="text-center flex-shrink-0">
              <p className={`text-lg font-bold ${(score.score_variaveis || 0) < 0 ? 'text-green-600' : 'text-amber-600'}`}>{score.score_variaveis ?? '?'}</p>
              <p className="text-[9px] text-[var(--pagsmile-blue)]/50">C2 Variáveis</p>
            </div>
            <span className="text-lg text-[var(--pagsmile-blue)]/30">+</span>
            <div className="text-center flex-shrink-0">
              <p className={`text-lg font-bold ${(score.score_enriquecimento || 0) < 0 ? 'text-green-600' : 'text-amber-600'}`}>{score.score_enriquecimento ?? '?'}</p>
              <p className="text-[9px] text-[var(--pagsmile-blue)]/50">C3 Enriquec.</p>
            </div>
            <ArrowRight className="w-5 h-5 text-[var(--pagsmile-blue)]/30 flex-shrink-0" />
            <div className="text-center flex-shrink-0">
              <p className="text-xl font-black text-[var(--pagsmile-blue)]">{score.score_final ?? '?'}</p>
              <p className="text-[9px] text-[var(--pagsmile-blue)]/50">Score Final</p>
            </div>
          </div>
        ...
        <>
          ...
          <div className="space-y-2">
            ...
            {score.score_geral_composto != null && (
              <div className="p-3 bg-[var(--pagsmile-blue)]/5 rounded-lg text-center">
                <p className="text-xs text-[var(--pagsmile-blue)]/50">Score Geral Composto (SGC)</p>
                <p className="text-2xl font-black text-[var(--pagsmile-blue)]">{score.score_geral_composto}</p>
                {score.classificacao_geral && <p className="text-[10px] text-[var(--pagsmile-blue)]/40">{score.classificacao_geral}</p>}
              </div>
            )}
          </div>
        </>
