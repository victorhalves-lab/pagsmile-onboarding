...
export default function MCCRateEditor({ mccItem, index, onChange, onRemove }) {
  ...
  return (
    <div className="rounded-xl border border-[#0A0A0A]/10 overflow-hidden">
      <div className="bg-[#f4f4f4] px-3 py-2 flex items-center gap-2">
        <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-2 flex-1 text-left">
          {expanded ? <ChevronDown className="w-3.5 h-3.5 text-[#0A0A0A]/30" /> : <ChevronRight className="w-3.5 h-3.5 text-[#0A0A0A]/30" />}
          <Badge className="bg-[#0A0A0A]/10 text-[#0A0A0A] border-0 text-[10px] font-mono">{mccItem.mccCode || '????'}</Badge>
          <span className="text-xs font-semibold text-[#0A0A0A]">{mccItem.mccDescription || 'Sem descrição'}</span>
        </button>
        ...
        <div className="p-3 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <Input
              ...
              className="border-[#0A0A0A]/10 h-8 text-xs"
            />
            <Input
              ...
              <tbody>
                {BANDEIRAS.map(b => (
                  <tr key={b} className="border-t border-[#0A0A0A]/[0.03]">
                    <td className="py-1.5 px-2 font-semibold text-[#0A0A0A] text-xs">{BANDEIRA_LABELS[b]}</td>
                    {FAIXA_COLS.map(f => (
                      <td key={f.key} className="py-1 px-1">
                        <BrNumberInput
                          ...
                          decimals={4}
                          className="border-[#0A0A0A]/10 h-7 text-[10px] text-center w-full"
                        />
                      </td>
                    ))}
                  </tr>
