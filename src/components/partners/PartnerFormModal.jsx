        ...
        <div className="space-y-6 py-2">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-1.5 md:col-span-2">
              ...
              <Input value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="Ex: Pagar.me, DOCK..." className="border-[#0A0A0A]/10" />
            </div>

            <div className="space-y-1.5">
              <Label className="text-xs text-[#0A0A0A]/50">Modelo</Label>
              <Input value={form.modelo} onChange={(e) => update('modelo', e.target.value)} placeholder="Ex: MDR fixo bandeira×MCC" className="border-[#0A0A0A]/10" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-[#0A0A0A]/50">Parcelas Máx</Label>
              <Input value={form.parcelasMax} onChange={(e) => update('parcelasMax', e.target.value)} placeholder="Ex: 12x, 21x" className="border-[#0A0A0A]/10" />
            </div>

            <div className="flex items-center gap-3 p-3 rounded-xl border border-[#0A0A0A]/5 bg-[#f4f4f4] md:col-span-2">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-[10px] text-[#0A0A0A]/40 font-bold uppercase tracking-wider">Taxas MDR por MCC</p>
              <Button variant="outline" size="sm" onClick={addMcc} className="text-xs h-7 rounded-lg border-[#0A0A0A]/10">
                <Plus className="w-3 h-3 mr-1" /> Adicionar MCC
              </Button>
            </div>
            {form.mdrByMcc.length === 0 && (
              <p className="text-xs text-[#0A0A0A]/30 text-center py-6">Nenhum MCC adicionado. Clique em "Adicionar MCC".</p>
            )}
          ...
          {/* Taxas adicionais */}
          <div>
            <p className="text-[10px] text-[#0A0A0A]/40 font-bold uppercase tracking-wider mb-3">Taxas Adicionais</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={f.key} className="space-y-1.5">
                  <Label className="text-xs text-[#0A0A0A]/50">{f.label}</Label>
                  <BrNumberInput value={form[f.key]} onChange={(val) => update(f.key, val)} decimals={2} className="border-[#0A0A0A]/10" />
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Antecipação Info */}
          <div className="space-y-1.5">
            <Label className="text-xs text-[#0A0A0A]/50">Info Antecipação</Label>
            <Input value={form.antecipacaoInfo} onChange={(e) => update('antecipacaoInfo', e.target.value)} placeholder="Ex: 120% CDI (~1,40% a.m.)" className="border-[#0A0A0A]/10" />
          </div>

          {/* Observações */}
          <div className="space-y-1.5">
            <Label className="text-xs text-[#0A0A0A]/50">Observações</Label>
            <Textarea value={form.notes} onChange={(e) => update('notes', e.target.value)} placeholder="Notas internas..." rows={2} className="border-[#0A0A0A]/10" />
          </div>
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} className="rounded-xl border-[#0A0A0A]/10">Cancelar</Button>
          <Button onClick={handleSave} disabled={saving || !form.name} className="bg-[#1356E2] hover:bg-[#1356E2]/90 text-white rounded-xl">
