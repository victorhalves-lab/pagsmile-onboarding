...
const CAPABILITIES = [
  ...
  { icon: Shield, title: 'Subadquirente Licenciada', desc: 'Credenciada Visa, Mastercard, Elo e Amex para processamento de cartões', gradient: 'from-[#1356E2] to-emerald-600' },
  ...
  { icon: Zap, title: 'PIX Instantâneo', desc: 'API PIX completa: QR Code estático/dinâmico, cobrança, transferência e conciliação', gradient: 'from-[#1356E2] to-teal-500' },
  ...
  { icon: Globe, title: 'Cross-Border Payments', desc: 'Processamento internacional para LATAM, Europa e Ásia com liquidação local', gradient: 'from-[#0A0A0A] to-blue-800' },
];
  ...
  return (
    <SlideLayout slideNumber={slideNumber} totalSlides={totalSlides}>
      <div className="flex items-start justify-between mb-3">
        <div>
          <h2 className="text-xl font-extrabold text-[#0A0A0A] tracking-tight">Quem é a Pin Bank</h2>
          <p className="text-[10px] text-[#0A0A0A]/40 mt-0.5">Infraestrutura completa de pagamentos para o seu negócio</p>
        </div>
        <div className="flex gap-1 mt-1">
          {BADGES.map((b, i) => (
            <span key={i} className="text-[7px] px-2 py-1 bg-[#0A0A0A]/[0.03] rounded-full text-[#0A0A0A]/40 font-medium whitespace-nowrap">{b}</span>
          ))}
        </div>
      </div>
        ...
        {CAPABILITIES.map((cap, i) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="text-[11px] font-bold text-[#0A0A0A] mb-1 tracking-tight">{cap.title}</h3>
              <p className="text-[8.5px] text-[#0A0A0A]/50 leading-[1.5]">{cap.desc}</p>
            </motion.div>
          );
        })}
      </div>
