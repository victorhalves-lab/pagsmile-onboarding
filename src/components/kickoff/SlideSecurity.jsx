...
const SECURITY_ITEMS = [
  { icon: Shield, title: 'PCI-DSS Level 1', desc: 'Certificação máxima de segurança para processamento de dados de cartão.', gradient: 'from-[#2bc196] to-emerald-600' },
  { icon: Lock, title: '3D Secure 2.0', desc: 'Autenticação forte do portador com experiência fluida e redução de chargebacks.', gradient: 'from-[#002443] to-blue-700' },
  ...
  { icon: AlertTriangle, title: 'Monitoramento 24/7', desc: 'Alertas proativos de anomalias, picos de chargeback e comportamento suspeito em tempo real.', gradient: 'from-[#002443] to-[#003a6b]' },
];

export default function SlideSecurity({ slideNumber, totalSlides }) {
  return (
    <SlideLayout slideNumber={slideNumber} totalSlides={totalSlides}>
      <h2 className="text-xl font-extrabold text-[#002443] mb-0.5">Segurança & Prevenção à Fraude</h2>
      <p className="text-[10px] text-[#002443]/60 mb-3">Camadas de proteção que garantem a segurança da sua operação</p>
        ...
        {SECURITY_ITEMS.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="text-[11px] font-bold text-[#002443] mb-1 tracking-tight">{item.title}</h3>
              <p className="text-[8.5px] text-[#002443]/50 leading-[1.5]">{item.desc}</p>
            </motion.div>
          );
        })}
      </div>
      ...
      <div className="mt-2 flex items-center justify-center gap-2">
        {['🔒 PCI-DSS Level 1', '📋 LGPD Compliant', '🏛️ BACEN Regulada', '🛡️ Tokenização de Dados'].map((badge, i) => (
          <span key={i} className="text-[8px] px-2.5 py-1 bg-[#002443]/[0.04] rounded-full text-[#002443]/50 font-medium">{badge}</span>
        ))}
      </div>
    </SlideLayout>
  );
