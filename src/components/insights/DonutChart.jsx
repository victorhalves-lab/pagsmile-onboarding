...
const FALLBACK = ['#1356E2', '#0A0A0A', '#E84B1C', '#E84B1C', '#94a3b8', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899'];
...
function renderActiveShape(props) {
  ...
  return (
    <g>
      ...
      <text x={cx} y={cy - 8} textAnchor="middle" fill="#0A0A0A" fontSize={18} fontWeight={800} letterSpacing="-0.02em">{value}</text>
      <text x={cx} y={cy + 12} textAnchor="middle" fill="#0A0A0A" fontSize={10} opacity={0.35} fontWeight={600}>{`${(percent * 100).toFixed(0)}%`}</text>
    </g>
  );
}
...
export default function DonutChart({ title, data, colorMap = {}, height = 240 }) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl bg-white/80 backdrop-blur-sm border border-white/60 p-5 transition-all duration-500 hover:shadow-[0_8px_40px_-12px_rgba(0,36,67,0.1)]">
      {/* Hover glow */}
      <div className="absolute -bottom-8 -right-8 w-20 h-20 bg-[#1356E2]/5 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-2 mt-1">
          {data.map((entry, i) => (
            ...
            >
              ...
              <span className="text-[10px] text-[#0A0A0A]/40 group-hover/legend:text-[#0A0A0A] transition-colors font-medium">{entry.name}</span>
              <span className="text-[10px] font-bold text-[#0A0A0A]/20 tabular-nums">{entry.value}</span>
            </button>
          ))}
        </div>
      </div>
