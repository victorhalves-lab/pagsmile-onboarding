  ...
  if (revalidations.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-[#002443]/8 p-6 text-center">
        <Clock className="w-8 h-8 text-[#002443]/15 mx-auto mb-2" />
        <p className="text-sm text-[#002443]/50">Nenhuma consulta BDC registrada para este cliente</p>
        <p className="text-[10px] text-[#002443]/30 mt-1">Execute o Enriquecimento BDC na aba correspondente</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#002443]/8 p-5">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <div>
          <h4 className="text-sm font-bold text-[#002443]">Timeline de Consultas BDC</h4>
          <p className="text-[10px] text-[#002443]/40">{revalidations.length} consulta(s) registrada(s)</p>
        </div>
      </div>

      <div className="relative">
        {/* Vertical line */}
        <div className="absolute left-4 top-2 bottom-2 w-0.5 bg-[#002443]/8" />
            ...
            return (
              <div key={v.id} className="relative flex gap-4 pl-1">
                ...
                <div className={`relative z-10 w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${
                  isFirst ? 'bg-[#2bc196] ring-4 ring-[#2bc196]/20' :
                ...
                <div className={`flex-1 rounded-xl border p-3 ${
                  isFirst ? 'border-[#2bc196]/30 bg-[#2bc196]/5' :
                  ...
                  <div className="flex items-start justify-between mb-1.5">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {isFirst && <Badge className="bg-[#2bc196]/20 text-[#2bc196] text-[9px] border-0">Mais recente</Badge>}
                        {v.status && (
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    {data.oldScore != null && (
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-[#002443]/40">Score:</span>
                        <span className="text-xs font-mono text-[#002443]">{data.oldScore}</span>
                        <span className="text-[#002443]/30">→</span>
                        <span className="text-xs font-mono font-bold text-[#002443]">{data.newScore}</span>
                        <span className={`text-[10px] font-mono font-bold ${
                          delta > 0 ? 'text-red-600' : delta < 0 ? 'text-green-600' : 'text-[#002443]/40'
                        }`}>
                          ({delta >= 0 ? '+' : ''}{delta})
                        </span>
                      </div>
                    ...
                    {v.score != null && !data.oldScore && (
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-[#002443]/40">Score:</span>
                        <span className="text-xs font-mono font-bold text-[#002443]">{v.score}</span>
                      </div>
                    )}
                    {data.newSubfaixa && (
