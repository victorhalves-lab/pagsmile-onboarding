    ...
    <div className="border border-slate-200 rounded-lg overflow-hidden bg-white group relative">
      ...
      <div className="p-2.5 flex items-center justify-between">
        <div className="flex items-center gap-1.5 min-w-0">
          <Icon className="w-3.5 h-3.5 text-[#2bc196] shrink-0" />
          <p className="text-xs font-semibold text-[#002443] truncate">{label}</p>
        </div>
...
function MetadataChip({ label, value, highlight = false }) {
  ...
  return (
    <div className={`px-3 py-2 rounded-lg border ${highlight ? 'bg-[#2bc196]/10 border-[#2bc196]/30' : 'bg-slate-50 border-slate-200'}`}>
      <p className="text-[9px] font-bold uppercase tracking-wider text-[#002443]/50">{label}</p>
      <p className={`text-sm font-bold ${highlight ? 'text-[#2bc196]' : 'text-[#002443]'}`}>{value}</p>
    </div>
  );
}
...
export default function CafDocsSection({ documents = [], integrationLogs = [] }) {
  ...
  return (
    <div className="bg-white rounded-xl border-2 border-[#2bc196]/20 p-6 mb-6">
      {/* Header */}
      <div className="flex items-center justify-between mb-5 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-[#2bc196]/10">
            <ScanFace className="w-5 h-5 text-[#2bc196]" />
          </div>
          <div>
            <h3 className="text-base font-bold text-[#002443]">Docs CAF — Identidade Verificada</h3>
            <p className="text-xs text-[#002443]/60">Imagens capturadas e validadas pela CAF</p>
          </div>
        </div>
      ...
      {hasMetadata && (
        <div className="pt-4 border-t border-slate-100">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-xs font-bold text-[#002443] uppercase tracking-wider">Resultado da Validação CAF</h4>
          </div>
