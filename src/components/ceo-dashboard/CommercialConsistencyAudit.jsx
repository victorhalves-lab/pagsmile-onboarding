...
export default function CommercialConsistencyAudit({ audit }) {
      ...
      renderItem: (item) => (
        <div className="flex items-center justify-between gap-3 py-1.5 px-2 hover:bg-amber-50/50 rounded">
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium text-[#0A0A0A] truncate">{item.fullName || '(sem nome)'}</p>
            <p className="text-[10px] text-[#0A0A0A]/50">status atual: <code className="bg-amber-100 px-1 rounded">{item.status}</code></p>
          </div>
          <Link to={`${createPageUrl('LeadDetails')}?id=${item.id}`} className="text-[10px] text-[#1356E2] hover:underline flex items-center gap-1 shrink-0">
    ...
    },
    {
      ...
      renderItem: (item) => (
        <div className="flex items-center justify-between gap-3 py-1.5 px-2 hover:bg-purple-50/50 rounded">
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium text-[#0A0A0A] truncate">{item.clienteNome || '(sem nome)'} <span className="text-[#0A0A0A]/40">· {item.codigo || item.id}</span></p>
            <p className="text-[10px] text-[#0A0A0A]/50">CNPJ: {item.clienteCnpj}</p>
          </div>
  ...
  return (
    <Card className="rounded-2xl border-[#0A0A0A]/5 shadow-sm">
      <CardContent className="p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-[#0A0A0A]">Auditoria de Consistência Comercial</h3>
              <p className="text-[11px] text-[#0A0A0A]/50">Discrepâncias entre Lead, Proposta e StandardProposal</p>
            </div>
          </div>
          {totalIssues === 0 ? (
        ...
        {totalIssues === 0 ? (
          <p className="text-xs text-[#0A0A0A]/60 text-center py-6">
          ...
          <div className="space-y-2">
            {sections.map(sec => {
              ...
              const Icon = sec.icon;
              return (
                <div key={sec.key} className="border border-[#0A0A0A]/5 rounded-xl overflow-hidden">
                  <button
                    onClick={() => setOpenSection(isOpen ? null : sec.key)}
                    className="w-full flex items-center justify-between gap-3 p-3 hover:bg-[#F7F5F0]/50 transition-colors"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="text-left min-w-0">
                        <p className="text-xs font-semibold text-[#0A0A0A]">{sec.title}</p>
                        <p className="text-[10px] text-[#0A0A0A]/50 truncate">{sec.subtitle}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <Badge variant="outline" className="text-[10px]">{sec.items.length}</Badge>
                      {isOpen ? <ChevronUp className="w-4 h-4 text-[#0A0A0A]/40" /> : <ChevronDown className="w-4 h-4 text-[#0A0A0A]/40" />}
                    </div>
                  </button>
                  {isOpen && (
                    <div className="border-t border-[#0A0A0A]/5 max-h-64 overflow-y-auto p-2 bg-[#F7F5F0]/30">
                      {sec.items.slice(0, 25).map((item, idx) => (
                        <div key={item.id || idx}>{sec.renderItem(item)}</div>
                      ))}
                      {sec.items.length > 25 && (
                        <p className="text-[10px] text-[#0A0A0A]/40 text-center mt-2">+ {sec.items.length - 25} adicional{sec.items.length - 25 > 1 ? 'is' : ''}</p>
                      )}
                    </div>
                  )}
                </div>
