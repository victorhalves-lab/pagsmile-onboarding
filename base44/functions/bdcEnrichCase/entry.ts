...
function analyzeIdentity(result) {
  ...
  // Tax status
  const status = safeGet(bd, 'TaxIdStatus') || safeGet(bd, 'TaxIdStatusDescription') || '';
  items.push({ label: 'Situação cadastral', value: String(status), risk: String(status).toUpperCase().includes('ATIV') ? 'OK' : 'CRITICO', points: 0 });
  ...
  if (taxRegime) items.push({ label: 'Regime tributário', value: `${taxRegime}${simples ? ' (Simples)' : ''}`, risk: 'INFO', points: 0 });

  // Capital
  const capital = safeGet(bd, 'ShareCapital') || safeGet(bd, 'Capital');
  if (capital != null) {
    const val = Number(capital);
    items.push({ label: 'Capital social', value: `R$ ${val.toLocaleString('pt-BR')}`, risk: val < 1000 ? 'ALTO' : val < 10000 ? 'MEDIO' : 'OK', points: val < 1000 ? 15 : val < 10000 ? 5 : 0 });
...
function analyzeCompliance(result) {
  ...
  if (collections) {
    ...
    if (hasCollections) {
      score += 30;
      items.push({ label: 'Em cobrança', value: 'SIM', risk: 'ALTO', points: 30 });
    } else {
...
function analyzePersonData(result) {
  ...
  if (bd) {
    ...
    sections.identity.items.push({ label: 'Situação CPF', value: first?.TaxIdStatus || 'N/D', risk: String(first?.TaxIdStatus || '').toUpperCase().includes('REGULAR') ? 'OK' : 'ALTO', points: 0 });
    if (first?.BirthDate) {
      const age = Math.floor((Date.now() - new Date(first.BirthDate).getTime()) / (365.25 * 24 * 3600 * 1000));
      sections.identity.items.push({ label: 'Idade', value: `${age} anos`, risk: age < 18 ? 'CRITICO' : 'OK', points: 0 });
    }
  ...
  if (pProcesses) {
    ...
    let total = 0;
    for (const item of pItems) { total += Number(item?.TotalLawsuits || 0); }
    sections.compliance.items.push({ label: 'Processos', value: total > 0 ? `${total} processo(s)` : 'Nenhum', risk: total > 5 ? 'ALTO' : total > 0 ? 'MEDIO' : 'OK', points: total > 5 ? 30 : total > 0 ? 10 : 0 });
    if (total > 0) sections.compliance.score += total > 5 ? 30 : 10;
  }
  ...
  if (pCollections) {
    ...
    sections.compliance.items.push({ label: 'Negativação', value: has ? 'SIM' : 'Não', risk: has ? 'ALTO' : 'OK', points: has ? 30 : 0 });
  ...
  if (pMedia) {
    ...
    let neg = 0;
    for (const item of mItems) {
      if (String(item?.Sentiment || '').toUpperCase().includes('NEGATIVE')) neg++;
    }
    sections.reputation.items.push({ label: 'Adverse media', value: neg > 0 ? `${neg} menção(ões) negativa(s)` : 'Nenhuma', risk: neg > 0 ? 'ALTO' : 'OK', points: neg > 0 ? 30 : 0 });
