...
async function sendSlack(token, channel, text, blocks) {
  const res = await fetch('https://slack.com/api/chat.postMessage', {
    ...
    body: JSON.stringify({ channel, text, blocks, username: 'Pagsmile Bot', icon_emoji: ':globe_with_meridians:' }),
  });
  return res.json();
}
...
Deno.serve(async (req) => {
  try {
    ...
    const blocks = [
      { type: 'header', text: { type: 'plain_text', text: '🌐 Novo Lead via Landing Page!', emoji: true } },
      { type: 'section', fields: [
    ...
    if (lead.introducerName) {
      blocks.push({ type: 'context', elements: [
        { type: 'mrkdwn', text: `👤 Introducer: *${lead.introducerName}* (slug: ${lead.slug || '-'})` }
      ]});
    }

    const { accessToken } = await base44.asServiceRole.connectors.getConnection('slackbot');
    const result = await sendSlack(accessToken, SLACK_CHANNEL,
      `🌐 Lead via Landing Page: ${lead.razaoSocial || 'N/A'} — ${lead.segment || ''}`, blocks);
