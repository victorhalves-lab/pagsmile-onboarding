 ...
 * Envia e-mail ao cliente com link público da proposta
 */
  ...
  try {
    ...
    const html = buildEmail({
      ...
      body: [
        ...
        `Caso tenha qualquer dúvida, estamos à disposição para agendar uma reunião e esclarecer todos os pontos.`,
        `Estamos ansiosos para tê-lo(a) conosco! 🚀`,
      ],
      ctaText: proposalUrl ? 'Ver Minha Proposta' : null,
      ctaUrl: proposalUrl,
      infoBox: `<p style="color:#166534;margin:0;font-size:13px"><strong>Código:</strong> ${proposal.codigo || 'N/A'}<br><strong>Segmento:</strong> ${proposal.businessSubCategory || 'N/A'}${proposal.responsavelNome ? `<br><strong>Consultor:</strong> ${proposal.responsavelNome}` : ''}</p>`,
      footerNote: proposal.mensagemEnvio || 'Obrigado por considerar a Pagsmile! 💚',
    });
