...
/**
 * RÉGUA #5 — E-mail: Fluxo Compliance (vários momentos)
 * Trigger: OnboardingCase [update]
 * Detecta mudanças de status e envia e-mails contextuais:
 * - Docs Solicitados: notifica merchant para enviar documentos
 * - Em Processamento: confirma que análise está em andamento
 * - Manual: informa que caso requer revisão manual
 */
Deno.serve(async (req) => {
  try {
    ...
    // Only handle specific compliance flow statuses (not Aprovado/Recusado — those are handled by notifyMerchantDecision)
    ...
    if (newStatus === 'Docs Solicitados') {
      // Build doc upload link if available
      ...
      emailConfig = {
        ...
        subtitle: 'Precisamos de alguns documentos para continuar sua análise.',
        greeting: `Olá, ${merchantName}!`,
        body: [
          `Sua análise de compliance está avançando! Para dar continuidade, precisamos que você nos envie alguns documentos complementares.`,
          `O envio é 100% digital e seguro. Basta clicar no botão abaixo para acessar a página de upload.`,
          `<strong>Documentos típicos:</strong> Contrato Social, Comprovante de Endereço, Documentos dos Sócios.`,
          `Quanto antes recebermos, mais rápido concluímos a análise. Estamos aqui para ajudar! 💪`,
        ],
        ctaText: docUrl ? 'Enviar Documentos' : null,
        ctaUrl: docUrl,
        subject: `📄 ${merchantName}, precisamos de alguns documentos`,
        fromName: 'Pagsmile Compliance',
      };
      ...
      emailConfig = {
        title: 'Revisão em Andamento 👁️',
        subtitle: 'Sua análise requer atenção especializada.',
        greeting: `Olá, ${merchantName}!`,
        body: [
          ...
          `Obrigado pela paciência! Estamos trabalhando para dar a melhor resposta possível. 🤝`,
        ],
