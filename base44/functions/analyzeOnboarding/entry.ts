...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const bdcDataString = bdcFullData ? truncateForPrompt(bdcFullData, 10000) : null;
    ...
    const cafDataString = cafFullData ? truncateForPrompt(cafFullData, 8000) : null;
    ...
    // ═══ BUILD PROMPT WITH FULL DATA — SEMANTIC RESPONSE FORMATTING ═══
    const formattedResponses = responses.map(r => {
      const tipo = r.questionType || 'TEXT';
      const pergunta = r.questionText || `Pergunta ${r.questionId}`;
      
      // Determine if question was answered at all
      ...
      // Semantic translation based on question type
      let resposta_semantica = '';
      let valor_bruto = '';

      if (tipo === 'BOOLEAN') {
        valor_bruto = r.valueBoolean ? 'SIM' : 'NÃO';
        // Determine semantic meaning based on question context
        const perguntaLower = pergunta.toLowerCase();
        const isNegativeQuestion = perguntaLower.includes('restrit') || perguntaLower.includes('proibid') || 
          perguntaLower.includes('criptomoeda') || perguntaLower.includes('jogos') || perguntaLower.includes('apostas') ||
          perguntaLower.includes('sancion') || perguntaLower.includes('pendência') || perguntaLower.includes('judicial') ||
          perguntaLower.includes('litígio') || perguntaLower.includes('irregularidade') || perguntaLower.includes('infração') ||
          perguntaLower.includes('processos') || perguntaLower.includes('condenação') || perguntaLower.includes('bloqueio');
        const isPositiveQuestion = perguntaLower.includes('compliance') || perguntaLower.includes('pld') || 
          perguntaLower.includes('monitora') || perguntaLower.includes('política') || perguntaLower.includes('programa') ||
          perguntaLower.includes('possui') && (perguntaLower.includes('certificação') || perguntaLower.includes('segurança')) ||
          perguntaLower.includes('auditor') || perguntaLower.includes('controle');
        const isPEPQuestion = perguntaLower.includes('pep') || perguntaLower.includes('pessoa exposta') || perguntaLower.includes('politicamente');

        if (r.valueBoolean === false) {
          if (isNegativeQuestion) {
            resposta_semantica = `NÃO — O merchant NEGA esta condição. Esta é uma resposta POSITIVA do ponto de vista de compliance (ausência de risco). Pergunta respondida completamente.`;
          } else if (isPEPQuestion) {
            resposta_semantica = `NÃO — O merchant declara que NÃO há PEPs envolvidos. Deve ser cross-validado com dados BDC/CAF, mas a resposta em si é COMPLETA e VÁLIDA.`;
          } else if (isPositiveQuestion) {
            resposta_semantica = `NÃO — O merchant declara NÃO possuir este controle/programa. Ponto de atenção legítimo: ausência de controle declarada pelo próprio merchant.`;
          } else {
            resposta_semantica = `NÃO — Resposta negativa. A pergunta foi respondida completamente.`;
          }
        } else {
          if (isNegativeQuestion) {
            resposta_semantica = `SIM — O merchant CONFIRMA esta condição de risco. RED FLAG potencial que deve ser investigado com dados BDC/CAF.`;
          } else if (isPEPQuestion) {
            resposta_semantica = `SIM — O merchant declara existência de PEP. Deve ser verificado com screening BDC/CAF para detalhamento.`;
          } else if (isPositiveQuestion) {
            resposta_semantica = `SIM — O merchant confirma possuir este controle/programa. Ponto POSITIVO de compliance.`;
          } else {
            resposta_semantica = `SIM — Resposta afirmativa. A pergunta foi respondida completamente.`;
          }
        }
      } else if (tipo === 'NUMBER') {
        valor_bruto = String(r.valueNumber);
        resposta_semantica = `Valor numérico informado: ${r.valueNumber}. Pergunta respondida completamente.`;
      } else if (tipo === 'SELECT') {
        valor_bruto = r.valueText || '';
        resposta_semantica = `Opção selecionada: "${valor_bruto}". Pergunta respondida completamente.`;
      } else if (tipo === 'MULTI_SELECT') {
        valor_bruto = r.valueArray?.join(', ') || r.valueText || '';
        const count = r.valueArray?.length || 0;
        resposta_semantica = `${count} opções selecionadas: [${valor_bruto}]. Pergunta respondida completamente.`;
      } else if (tipo === 'CPF_CNPJ') {
        valor_bruto = r.valueText || '';
        resposta_semantica = `Documento informado: ${valor_bruto}. Deve ser cross-validado com dados BDC.`;
      } else if (tipo === 'EMAIL') {
        valor_bruto = r.valueText || '';
        resposta_semantica = `Email informado: ${valor_bruto}. Deve ser cross-validado com dados BDC.`;
      } else if (tipo === 'PHONE') {
        valor_bruto = r.valueText || '';
        resposta_semantica = `Telefone informado: ${valor_bruto}. Deve ser cross-validado com dados BDC.`;
      } else {
        // TEXT, DATE, FILE_UPLOAD, etc.
        valor_bruto = r.valueText || '';
        if (valor_bruto.trim().length < 3 || ['n/a', 'na', '-', '.', '...'].includes(valor_bruto.trim().toLowerCase())) {
          resposta_semantica = `Resposta vazia ou genérica: "${valor_bruto}" — pode indicar evasão ou desinteresse. Lacuna parcial.`;
        } else {
          resposta_semantica = `Texto informado: "${valor_bruto}". Pergunta respondida.`;
        }
      }
...
ATENÇÃO — REGRAS DE INTERPRETAÇÃO DO QUESTIONÁRIO:
1. Cada resposta inclui "status" (RESPONDIDA ou NÃO RESPONDIDA) e "resposta_semantica" com o significado correto.
2. Uma resposta SIM/NÃO (BOOLEAN) que consta "RESPONDIDA" É uma resposta COMPLETA. NÃO a trate como lacuna.
3. "NÃO" em perguntas sobre atividades proibidas/restritas/cripto/jogos/PEP é a resposta DESEJADA. É um ponto POSITIVO.
4. "SIM" em perguntas sobre compliance/PLD/monitoramento/programa/política é a resposta DESEJADA. É um ponto POSITIVO.
5. Somente marque como "lacuna" ou "informação ausente" respostas com status "NÃO RESPONDIDA" ou resposta_semantica contendo "lacuna".
6. NUNCA diga "as respostas carecem de informações" quando as perguntas estão marcadas como RESPONDIDAS.
7. Para perguntas BOOLEAN, a resposta "NÃO" é TÃO VÁLIDA quanto "SIM" — ambas são respostas completas.
8. Se quiser questionar a VERACIDADE de uma resposta (ex: "disse NÃO para PEP mas BDC mostra IsPEP=true"), isso é um achado de CROSS-VALIDATION, não uma lacuna no questionário.

RESUMO ESTATÍSTICO DO QUESTIONÁRIO:
- Total de perguntas: ${formattedResponses.length}
- Respondidas: ${formattedResponses.filter(r => r.status === 'RESPONDIDA').length}
- Não respondidas: ${formattedResponses.filter(r => r.status === 'NÃO RESPONDIDA').length}
- Booleanas (Sim/Não): ${formattedResponses.filter(r => r.tipo === 'BOOLEAN').length} (${formattedResponses.filter(r => r.tipo === 'BOOLEAN' && r.status === 'RESPONDIDA').length} respondidas)
- Textuais: ${formattedResponses.filter(r => r.tipo === 'TEXT').length}
- Numéricas: ${formattedResponses.filter(r => r.tipo === 'NUMBER').length}
- Seleção: ${formattedResponses.filter(r => r.tipo === 'SELECT' || r.tipo === 'MULTI_SELECT').length}
- Taxa de completude: ${formattedResponses.length > 0 ? ((formattedResponses.filter(r => r.status === 'RESPONDIDA').length / formattedResponses.length) * 100).toFixed(1) : 0}%

RESPOSTAS FORMATADAS (compactadas):
${truncateForPrompt(formattedResponses.map(r => `[${r.status}][${r.tipo}] "${r.pergunta}" → ${r.resposta_semantica}`), 12000)}
...
0. **ANÁLISE DO QUESTIONÁRIO** — REGRAS OBRIGATÓRIAS:
   a) Leia o campo "resposta_semantica" de cada resposta — ele contém a interpretação CORRETA.
   b) Uma resposta BOOLEAN com status "RESPONDIDA" é COMPLETA. Não a trate como lacuna/incompletude.
   c) "NÃO opera com criptomoedas" = POSITIVO (ausência de risco). "SIM possui compliance" = POSITIVO.
   d) Lacuna REAL = pergunta com status "NÃO RESPONDIDA" ou resposta genérica (N/A, ponto, traço).
   e) Se o questionário tem taxa de completude > 80%, NÃO diga "respostas carecem de informações cruciais".
   f) Para questionar a veracidade (ex: declarou NÃO PEP mas BDC diz SIM), use CROSS-VALIDATION, não "lacuna".
   g) Na análise completa, liste as respostas Sim/Não como FATOS DECLARADOS pelo merchant, não como falhas.
